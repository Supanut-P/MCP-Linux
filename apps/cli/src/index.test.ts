...
import { err } from '@baitonghub-linux-mcp/domain';
...
describe('CLI argument parser', () => {
  it('parses workspace, MCP, doctor, and Codex doctor commands', () => {
    expect(parseCliArgs(['workspace', 'add', '/srv/project'])).toEqual({ ok: true, value: { kind: 'workspace-add', rootPath: '/srv/project' } });
  ...
  it('prints sanitized Codex discovery diagnostics from codex doctor', async () => {
    ...
    expect(output).toEqual(['Codex --version check failed (stage=--version, executable=/opt/codex/bin/codex, spawnErrorCode=EACCES, exitCode=-1)']);
  });
});

...
import { formatCodexDiscoveryError, type CodexDiscoveryResult } from '@baitonghub-linux-mcp/codex';
import type { DoctorReport } from '@baitonghub-linux-mcp/application';
...
export function parseCliArgs(args: readonly string[]): Result<CliCommand> {
  ...
  return err(appError('INVALID_INPUT', 'Unknown baitonghub-linux-mcp command'));
}
...
function parseWorkspaceArgs(args: readonly string[]): Result<CliCommand> {
  ...
  return err(appError('INVALID_INPUT', 'Usage: baitonghub-linux-mcp workspace add <path> | workspace list'));
}
...
function parseMcpArgs(args: readonly string[]): Result<CliCommand> {
  ...
  for (let index = 1; index < args.length; index += 1) {
    ...
    if (flag === '--stdio' || flag === '--http') {
      ...
      return err(appError('INVALID_INPUT', 'Usage: baitonghub-linux-mcp mcp --stdio|--http [--workspace <id-or-path>]'));
    }
  }

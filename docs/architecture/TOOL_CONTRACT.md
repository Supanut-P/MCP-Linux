...
| 69 | `db_inspect` | READ | yes | no |
| 70 | `db_query` | READ | yes | no |
| 71 | `target_catalog` | READ | yes | no |
| 72 | `support_bundle` | DANGEROUS | no | yes |
| 73 | `skills_list` | DANGEROUS | no | yes |
| 74 | `skills_read` | DANGEROUS | no | yes |
| 75 | `mcp_list` | DANGEROUS | no | yes |
| 76 | `mcp_describe` | DANGEROUS | no | yes |
| 77 | `mcp_call` | DANGEROUS | no | yes |
| 78 | `workspace_context` | READ | yes | no |
| 79 | `workspace_context_continue` | READ | yes | no |
| 80 | `workspace_full_scan` | READ | yes | no |
| 81 | `workspace_full_scan_continue` | READ | yes | no |
| 82 | `workspace_snapshot` | READ | yes | no |
| 83 | `search_all` | READ | yes | no |
| 84 | `read_many_files` | READ | yes | no |
| 85 | `read_file_page` | READ | yes | no |
| 86 | `read_file_page_continue` | READ | yes | no |
| 87 | `workspace_index` | READ | yes | no |
| 88 | `workspace_index_status` | READ | yes | no |
| 89 | `workspace_index_watch` | READ | yes | no |
| 90 | `workspace_index_stop` | READ | yes | no |
| 91 | `workspace_changes` | READ | yes | no |
| 92 | `session_handoff` | READ | yes | no |
| 93 | `verify_incremental` | EXECUTE | no | no |
| 94 | `symbol_search` | READ | yes | no |
| 95 | `find_definition` | READ | yes | no |
| 96 | `find_references` | READ | yes | no |
| 97 | `find_implementations` | READ | yes | no |
| 98 | `call_hierarchy` | READ | yes | no |
| 99 | `import_graph` | READ | yes | no |
| 100 | `dependency_graph` | READ | yes | no |
| 101 | `module_graph` | READ | yes | no |
| 102 | `type_search` | READ | yes | no |
| 103 | `trace_symbol` | READ | yes | no |
| 104 | `context_ranking` | READ | yes | no |
| 105 | `debug_context` | READ | yes | no |
| 106 | `review_context` | READ | yes | no |
| 107 | `change_context` | READ | yes | no |
| 108 | `symbol_context` | READ | yes | no |
| 109 | `test_context` | READ | yes | no |
| 110 | `dependency_context` | READ | yes | no |
| 111 | `git_context` | READ | yes | no |
| 112 | `frontend_context` | READ | yes | no |
| 113 | `backend_context` | READ | yes | no |
| 114 | `route_intent` | READ | yes | no |
| 115 | `recipe_list` | READ | yes | no |
| 116 | `recipe_describe` | READ | yes | no |
| 117 | `recipe_run` | EXECUTE | no | no |
| 118 | `dry_run` | READ | yes | no |
| 119 | `review_changes` | READ | yes | no |
| 120 | `changed_symbols` | READ | yes | no |
| 121 | `affected_modules` | READ | yes | no |
| 122 | `git_history_context` | READ | yes | no |
| 123 | `git_blame_context` | READ | yes | no |
| 124 | `discover_tests` | READ | yes | no |
| 125 | `run_affected_tests` | EXECUTE | no | no |
| 126 | `test_failures` | READ | yes | no |
| 127 | `coverage_context` | READ | yes | no |
| 128 | `test_history` | READ | yes | no |
| 129 | `cache_stats` | READ | yes | no |
| 130 | `cache_clear` | WRITE | no | no |
| 131 | `cache_invalidate` | WRITE | no | no |
| 132 | `hook_list` | READ | yes | no |
| 133 | `hook_register` | WRITE | no | no |
| 134 | `hook_remove` | WRITE | no | no |
| 135 | `skill_match` | READ | yes | no |
| 136 | `skill_load` | READ | yes | no |
| 137 | `plugin_install` | DANGEROUS | no | yes |
| 138 | `plugin_list` | READ | yes | no |
| 139 | `plugin_enable` | WRITE | no | no |
| 140 | `plugin_disable` | WRITE | no | no |
| 141 | `plugin_remove` | DANGEROUS | no | yes |
| 142 | `session_context` | READ | yes | no |
| 143 | `session_checkpoint` | WRITE | no | no |
| 144 | `session_resume` | READ | yes | no |
| 145 | `session_history` | READ | yes | no |
| 146 | `response_mode` | READ | yes | no |
| 147 | `inspect_web_app` | READ | yes | no |
| 148 | `debug_ui` | READ | yes | no |
| 149 | `capture_ui_state` | READ | yes | no |
| 150 | `form_context` | READ | yes | no |
| 151 | `network_context` | READ | yes | no |
| 152 | `console_context` | READ | yes | no |
| 153 | `browser_debug_context` | READ | yes | no |
...
| 200 | `inspect_pdf` | READ | yes | no |
| 201 | `compare_pdf_pages` | READ | yes | no |
| 202 | `project_profile_get` | READ | yes | no |
| 203 | `project_profile_set` | WRITE | no | no |
| 204 | `handoff_context` | READ | yes | no |
| 205 | `benchmark_run` | EXECUTE | no | no |
| 206 | `regression_report` | READ | yes | no |
...
| 213 | `pdf_extract_tables` | READ | yes | no |

...
This block is generated from the built `ToolRegistry`. Current count: **215 tools**.
...
| 47 | `service_logs` | READ | yes | no |
| 48 | `network` | READ | yes | no |
| 49 | `service` | DANGEROUS | no | yes |
| 50 | `package` | DANGEROUS | no | yes |
| 51 | `schedule` | DANGEROUS | no | yes |
| 52 | `notification` | EXECUTE | no | no |
| 53 | `file_dialog` | EXECUTE | yes | no |
| 54 | `clipboard` | DANGEROUS | no | no |
| 55 | `web_fetch` | DANGEROUS | yes | no |
| 56 | `container` | DANGEROUS | no | yes |
| 57 | `archive` | DANGEROUS | no | yes |
| 58 | `dependency_audit` | READ | yes | no |
| 59 | `remote_host` | DANGEROUS | no | yes |
| 60 | `remote_fleet` | READ | yes | no |
| 61 | `remote_rollout` | DANGEROUS | no | yes |
| 62 | `remote_rollout_resume` | DANGEROUS | no | yes |
| 63 | `artifact_verify` | READ | yes | no |
| 64 | `http_probe` | READ | yes | no |
| 65 | `storage_usage` | READ | yes | no |
| 66 | `backup` | DANGEROUS | no | yes |
| 67 | `db_inspect` | READ | yes | no |
| 68 | `db_query` | READ | yes | no |
| 69 | `target_catalog` | READ | yes | no |
| 70 | `support_bundle` | DANGEROUS | no | yes |
| 71 | `skills_list` | DANGEROUS | no | yes |
| 72 | `skills_read` | DANGEROUS | no | yes |
| 73 | `mcp_list` | DANGEROUS | no | yes |
| 74 | `mcp_describe` | DANGEROUS | no | yes |
| 75 | `mcp_call` | DANGEROUS | no | yes |
| 76 | `workspace_context` | READ | yes | no |
| 77 | `workspace_context_continue` | READ | yes | no |
| 78 | `workspace_full_scan` | READ | yes | no |
| 79 | `workspace_full_scan_continue` | READ | yes | no |
| 80 | `workspace_snapshot` | READ | yes | no |
| 81 | `search_all` | READ | yes | no |
| 82 | `read_many_files` | READ | yes | no |
| 83 | `read_file_page` | READ | yes | no |
| 84 | `read_file_page_continue` | READ | yes | no |
| 85 | `workspace_index` | READ | yes | no |
| 86 | `workspace_index_status` | READ | yes | no |
| 87 | `workspace_index_watch` | READ | yes | no |
| 88 | `workspace_index_stop` | READ | yes | no |
| 89 | `session_handoff` | READ | yes | no |
| 90 | `verify_incremental` | EXECUTE | no | no |
...
| 134 | `plugin_install` | DANGEROUS | no | yes |
...
| 154 | `installed_runtime_context` | READ | yes | no |
| 155 | `path_context` | READ | yes | no |
| 156 | `startup_context` | READ | yes | no |
| 157 | `mcp_discover` | READ | yes | no |
| 158 | `mcp_health` | READ | yes | no |
| 159 | `mcp_resources` | READ | yes | no |
| 160 | `task_create` | EXECUTE | no | no |
| 161 | `task_status` | READ | yes | no |
| 162 | `task_cancel` | EXECUTE | no | no |
| 163 | `task_result` | READ | yes | no |
| 164 | `task_list` | READ | yes | no |
| 165 | `delegate` | EXECUTE | no | no |
| 166 | `delegate_status` | READ | yes | no |
| 167 | `delegate_cancel` | EXECUTE | no | no |
| 168 | `delegate_result` | READ | yes | no |
| 169 | `parallel_delegate` | EXECUTE | no | no |
| 170 | `permission_check` | READ | yes | no |
| 171 | `permission_profile` | READ | yes | no |
| 172 | `live_logs_query` | READ | yes | no |
| 173 | `live_logs_status` | READ | yes | no |
| 174 | `telemetry_dashboard` | READ | yes | no |
| 175 | `context_economy_stats` | READ | yes | no |
| 176 | `execution_plan` | READ | yes | no |
| 177 | `repo_map` | READ | yes | no |
| 178 | `context_expand` | READ | yes | no |
| 179 | `recovery_status` | READ | yes | no |
| 180 | `tool_schema_list` | READ | yes | no |
| 181 | `tool_schema_register` | WRITE | no | no |
| 182 | `capabilities` | READ | yes | no |
| 183 | `tool_search` | READ | yes | no |
| 184 | `tool_dynamic_filter` | READ | yes | no |
| 185 | `tool_describe` | READ | yes | no |
| 186 | `tool_categories` | READ | yes | no |
| 187 | `tool_function_find` | READ | yes | no |
| 188 | `tool_aliases` | READ | yes | no |
| 189 | `mcp_hub` | READ | yes | no |
| 190 | `dev_context` | READ | yes | no |
| 191 | `recipe_catalog` | READ | yes | no |
| 192 | `capture_screenshot` | READ | yes | no |
| 193 | `compare_screenshot` | READ | yes | no |
| 194 | `dom_snapshot` | READ | yes | no |
| 195 | `layout_metadata` | READ | yes | no |
| 196 | `visual_context` | READ | yes | no |
...
| 204 | `lsp_diagnostics` | READ | yes | no |
| 205 | `lsp_rename` | WRITE | no | no |
| 206 | `debug_attach` | EXECUTE | no | no |
| 207 | `debug_step` | EXECUTE | no | no |
| 208 | `git_worktree_spawn` | DANGEROUS | no | yes |
| 209 | `git_worktree_remove` | DANGEROUS | no | yes |
| 210 | `pdf_extract_tables` | READ | yes | no |
| 211 | `self_heal_plan` | READ | yes | no |
| 212 | `self_heal_apply` | DANGEROUS | no | yes |
| 213 | `skills_import` | WRITE | no | no |
| 214 | `agent_swarm_run` | EXECUTE | no | no |
| 215 | `tool_batch` | DANGEROUS | no | yes |

...
This block is generated from the built `ToolRegistry`. Current count: **226 tools**.
...
| 76 | `db_inspect` | READ | yes | no |
| 77 | `db_query` | READ | yes | no |
| 78 | `target_catalog` | READ | yes | no |
| 79 | `support_bundle` | DANGEROUS | no | yes |
| 80 | `skills_list` | DANGEROUS | no | yes |
| 81 | `skills_read` | DANGEROUS | no | yes |
| 82 | `mcp_list` | DANGEROUS | no | yes |
| 83 | `mcp_describe` | DANGEROUS | no | yes |
| 84 | `mcp_call` | DANGEROUS | no | yes |
| 85 | `workspace_context` | READ | yes | no |
| 86 | `workspace_context_continue` | READ | yes | no |
| 87 | `workspace_full_scan` | READ | yes | no |
| 88 | `workspace_full_scan_continue` | READ | yes | no |
| 89 | `workspace_snapshot` | READ | yes | no |
| 90 | `search_all` | READ | yes | no |
| 91 | `read_many_files` | READ | yes | no |
| 92 | `read_file_page` | READ | yes | no |
| 93 | `read_file_page_continue` | READ | yes | no |
| 94 | `workspace_index` | READ | yes | no |
| 95 | `workspace_index_status` | READ | yes | no |
| 96 | `workspace_index_watch` | READ | yes | no |
| 97 | `workspace_index_stop` | READ | yes | no |
| 98 | `workspace_changes` | READ | yes | no |
| 99 | `session_handoff` | READ | yes | no |
| 100 | `verify_incremental` | EXECUTE | no | no |
| 101 | `symbol_search` | READ | yes | no |
| 102 | `find_definition` | READ | yes | no |
| 103 | `find_references` | READ | yes | no |
| 104 | `find_implementations` | READ | yes | no |
| 105 | `call_hierarchy` | READ | yes | no |
| 106 | `import_graph` | READ | yes | no |
| 107 | `dependency_graph` | READ | yes | no |
| 108 | `module_graph` | READ | yes | no |
| 109 | `type_search` | READ | yes | no |
| 110 | `trace_symbol` | READ | yes | no |
| 111 | `context_ranking` | READ | yes | no |
| 112 | `debug_context` | READ | yes | no |
| 113 | `review_context` | READ | yes | no |
| 114 | `change_context` | READ | yes | no |
| 115 | `symbol_context` | READ | yes | no |
| 116 | `test_context` | READ | yes | no |
| 117 | `dependency_context` | READ | yes | no |
| 118 | `git_context` | READ | yes | no |
| 119 | `frontend_context` | READ | yes | no |
| 120 | `backend_context` | READ | yes | no |
| 121 | `route_intent` | READ | yes | no |
| 122 | `recipe_list` | READ | yes | no |
| 123 | `recipe_describe` | READ | yes | no |
| 124 | `recipe_run` | EXECUTE | no | no |
| 125 | `dry_run` | READ | yes | no |
| 126 | `review_changes` | READ | yes | no |
| 127 | `changed_symbols` | READ | yes | no |
| 128 | `affected_modules` | READ | yes | no |
| 129 | `git_history_context` | READ | yes | no |
| 130 | `git_blame_context` | READ | yes | no |
| 131 | `discover_tests` | READ | yes | no |
| 132 | `run_affected_tests` | EXECUTE | no | no |
| 133 | `test_failures` | READ | yes | no |
| 134 | `coverage_context` | READ | yes | no |
| 135 | `test_history` | READ | yes | no |
| 136 | `cache_stats` | READ | yes | no |
| 137 | `cache_clear` | WRITE | no | no |
| 138 | `cache_invalidate` | WRITE | no | no |
| 139 | `hook_list` | READ | yes | no |
| 140 | `hook_register` | WRITE | no | no |
| 141 | `hook_remove` | WRITE | no | no |
| 142 | `skill_match` | READ | yes | no |
| 143 | `skill_load` | READ | yes | no |
| 144 | `plugin_install` | DANGEROUS | no | yes |
| 145 | `plugin_list` | READ | yes | no |
| 146 | `plugin_enable` | WRITE | no | no |
| 147 | `plugin_disable` | WRITE | no | no |
| 148 | `plugin_remove` | DANGEROUS | no | yes |
| 149 | `session_context` | READ | yes | no |
| 150 | `session_checkpoint` | WRITE | no | no |
| 151 | `session_resume` | READ | yes | no |
| 152 | `session_history` | READ | yes | no |
| 153 | `response_mode` | READ | yes | no |
| 154 | `inspect_web_app` | READ | yes | no |
| 155 | `debug_ui` | READ | yes | no |
| 156 | `capture_ui_state` | READ | yes | no |
| 157 | `form_context` | READ | yes | no |
| 158 | `network_context` | READ | yes | no |
| 159 | `console_context` | READ | yes | no |
| 160 | `browser_debug_context` | READ | yes | no |
| 161 | `service_context` | READ | yes | no |
| 162 | `process_context` | READ | yes | no |
| 163 | `port_context` | READ | yes | no |
| 164 | `installed_runtime_context` | READ | yes | no |
| 165 | `path_context` | READ | yes | no |
| 166 | `startup_context` | READ | yes | no |
| 167 | `mcp_discover` | READ | yes | no |
| 168 | `mcp_health` | READ | yes | no |
| 169 | `mcp_resources` | READ | yes | no |
| 170 | `task_create` | EXECUTE | no | no |
| 171 | `task_status` | READ | yes | no |
| 172 | `task_cancel` | EXECUTE | no | no |
| 173 | `task_result` | READ | yes | no |
| 174 | `task_list` | READ | yes | no |
| 175 | `delegate` | EXECUTE | no | no |
| 176 | `delegate_status` | READ | yes | no |
| 177 | `delegate_cancel` | EXECUTE | no | no |
| 178 | `delegate_result` | READ | yes | no |
| 179 | `parallel_delegate` | EXECUTE | no | no |
| 180 | `permission_check` | READ | yes | no |
| 181 | `permission_profile` | READ | yes | no |
| 182 | `live_logs_query` | READ | yes | no |
| 183 | `live_logs_status` | READ | yes | no |
| 184 | `telemetry_dashboard` | READ | yes | no |
| 185 | `context_economy_stats` | READ | yes | no |
| 186 | `execution_plan` | READ | yes | no |
| 187 | `repo_map` | READ | yes | no |
| 188 | `context_expand` | READ | yes | no |
| 189 | `recovery_status` | READ | yes | no |
| 190 | `tool_schema_list` | READ | yes | no |
| 191 | `tool_schema_register` | WRITE | no | no |
| 192 | `capabilities` | READ | yes | no |
| 193 | `tool_search` | READ | yes | no |
| 194 | `tool_dynamic_filter` | READ | yes | no |
| 195 | `tool_describe` | READ | yes | no |
| 196 | `tool_categories` | READ | yes | no |
| 197 | `tool_function_find` | READ | yes | no |
| 198 | `tool_aliases` | READ | yes | no |
| 199 | `mcp_hub` | READ | yes | no |
| 200 | `dev_context` | READ | yes | no |
| 201 | `recipe_catalog` | READ | yes | no |
| 202 | `capture_screenshot` | READ | yes | no |
| 203 | `compare_screenshot` | READ | yes | no |
| 204 | `dom_snapshot` | READ | yes | no |
| 205 | `layout_metadata` | READ | yes | no |
| 206 | `visual_context` | READ | yes | no |
| 207 | `inspect_pdf` | READ | yes | no |
| 208 | `compare_pdf_pages` | READ | yes | no |
| 209 | `project_profile_get` | READ | yes | no |
| 210 | `project_profile_set` | WRITE | no | no |
| 211 | `handoff_context` | READ | yes | no |
| 212 | `benchmark_run` | EXECUTE | no | no |
| 213 | `regression_report` | READ | yes | no |
| 214 | `lsp_diagnostics` | READ | yes | no |
| 215 | `lsp_rename` | WRITE | no | no |
| 216 | `debug_attach` | EXECUTE | no | no |
| 217 | `debug_step` | EXECUTE | no | no |
| 218 | `git_worktree_spawn` | DANGEROUS | no | yes |
| 219 | `git_worktree_remove` | DANGEROUS | no | yes |
| 220 | `pdf_extract_tables` | READ | yes | no |
| 221 | `self_heal_plan` | READ | yes | no |
| 222 | `self_heal_apply` | DANGEROUS | no | yes |
| 223 | `skills_import` | WRITE | no | no |
| 224 | `agent_swarm_run` | EXECUTE | no | no |
| 225 | `policy_explain` | READ | yes | no |
| 226 | `tool_batch` | DANGEROUS | no | yes |

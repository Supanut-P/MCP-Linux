...
| 73 | `db_inspect` | READ | yes | no |
| 74 | `db_query` | READ | yes | no |
| 75 | `target_catalog` | READ | yes | no |
| 76 | `support_bundle` | DANGEROUS | no | yes |
| 77 | `skills_list` | DANGEROUS | no | yes |
| 78 | `skills_read` | DANGEROUS | no | yes |
| 79 | `mcp_list` | DANGEROUS | no | yes |
| 80 | `mcp_describe` | DANGEROUS | no | yes |
| 81 | `mcp_call` | DANGEROUS | no | yes |
| 82 | `workspace_context` | READ | yes | no |
| 83 | `workspace_context_continue` | READ | yes | no |
| 84 | `workspace_full_scan` | READ | yes | no |
| 85 | `workspace_full_scan_continue` | READ | yes | no |
| 86 | `workspace_snapshot` | READ | yes | no |
| 87 | `search_all` | READ | yes | no |
| 88 | `read_many_files` | READ | yes | no |
| 89 | `read_file_page` | READ | yes | no |
| 90 | `read_file_page_continue` | READ | yes | no |
| 91 | `workspace_index` | READ | yes | no |
| 92 | `workspace_index_status` | READ | yes | no |
| 93 | `workspace_index_watch` | READ | yes | no |
| 94 | `workspace_index_stop` | READ | yes | no |
| 95 | `workspace_changes` | READ | yes | no |
| 96 | `session_handoff` | READ | yes | no |
| 97 | `verify_incremental` | EXECUTE | no | no |
| 98 | `symbol_search` | READ | yes | no |
| 99 | `find_definition` | READ | yes | no |
| 100 | `find_references` | READ | yes | no |
| 101 | `find_implementations` | READ | yes | no |
| 102 | `call_hierarchy` | READ | yes | no |
| 103 | `import_graph` | READ | yes | no |
| 104 | `dependency_graph` | READ | yes | no |
| 105 | `module_graph` | READ | yes | no |
| 106 | `type_search` | READ | yes | no |
| 107 | `trace_symbol` | READ | yes | no |
| 108 | `context_ranking` | READ | yes | no |
| 109 | `debug_context` | READ | yes | no |
| 110 | `review_context` | READ | yes | no |
| 111 | `change_context` | READ | yes | no |
| 112 | `symbol_context` | READ | yes | no |
| 113 | `test_context` | READ | yes | no |
| 114 | `dependency_context` | READ | yes | no |
| 115 | `git_context` | READ | yes | no |
| 116 | `frontend_context` | READ | yes | no |
| 117 | `backend_context` | READ | yes | no |
| 118 | `route_intent` | READ | yes | no |
| 119 | `recipe_list` | READ | yes | no |
| 120 | `recipe_describe` | READ | yes | no |
| 121 | `recipe_run` | EXECUTE | no | no |
| 122 | `dry_run` | READ | yes | no |
| 123 | `review_changes` | READ | yes | no |
| 124 | `changed_symbols` | READ | yes | no |
| 125 | `affected_modules` | READ | yes | no |
| 126 | `git_history_context` | READ | yes | no |
| 127 | `git_blame_context` | READ | yes | no |
| 128 | `discover_tests` | READ | yes | no |
| 129 | `run_affected_tests` | EXECUTE | no | no |
| 130 | `test_failures` | READ | yes | no |
| 131 | `coverage_context` | READ | yes | no |
| 132 | `test_history` | READ | yes | no |
| 133 | `cache_stats` | READ | yes | no |
| 134 | `cache_clear` | WRITE | no | no |
| 135 | `cache_invalidate` | WRITE | no | no |
| 136 | `hook_list` | READ | yes | no |
| 137 | `hook_register` | WRITE | no | no |
| 138 | `hook_remove` | WRITE | no | no |
| 139 | `skill_match` | READ | yes | no |
| 140 | `skill_load` | READ | yes | no |
| 141 | `plugin_install` | DANGEROUS | no | yes |
| 142 | `plugin_list` | READ | yes | no |
| 143 | `plugin_enable` | WRITE | no | no |
| 144 | `plugin_disable` | WRITE | no | no |
| 145 | `plugin_remove` | DANGEROUS | no | yes |
| 146 | `session_context` | READ | yes | no |
| 147 | `session_checkpoint` | WRITE | no | no |
| 148 | `session_resume` | READ | yes | no |
| 149 | `session_history` | READ | yes | no |
| 150 | `response_mode` | READ | yes | no |
| 151 | `inspect_web_app` | READ | yes | no |
| 152 | `debug_ui` | READ | yes | no |
| 153 | `capture_ui_state` | READ | yes | no |
| 154 | `form_context` | READ | yes | no |
| 155 | `network_context` | READ | yes | no |
| 156 | `console_context` | READ | yes | no |
| 157 | `browser_debug_context` | READ | yes | no |
| 158 | `service_context` | READ | yes | no |
| 159 | `process_context` | READ | yes | no |
| 160 | `port_context` | READ | yes | no |
| 161 | `installed_runtime_context` | READ | yes | no |
| 162 | `path_context` | READ | yes | no |
| 163 | `startup_context` | READ | yes | no |
| 164 | `mcp_discover` | READ | yes | no |
| 165 | `mcp_health` | READ | yes | no |
| 166 | `mcp_resources` | READ | yes | no |
| 167 | `task_create` | EXECUTE | no | no |
| 168 | `task_status` | READ | yes | no |
| 169 | `task_cancel` | EXECUTE | no | no |
| 170 | `task_result` | READ | yes | no |
| 171 | `task_list` | READ | yes | no |
| 172 | `delegate` | EXECUTE | no | no |
| 173 | `delegate_status` | READ | yes | no |
| 174 | `delegate_cancel` | EXECUTE | no | no |
| 175 | `delegate_result` | READ | yes | no |
| 176 | `parallel_delegate` | EXECUTE | no | no |
| 177 | `permission_check` | READ | yes | no |
| 178 | `permission_profile` | READ | yes | no |
| 179 | `live_logs_query` | READ | yes | no |
| 180 | `live_logs_status` | READ | yes | no |
| 181 | `telemetry_dashboard` | READ | yes | no |
| 182 | `context_economy_stats` | READ | yes | no |
| 183 | `execution_plan` | READ | yes | no |
| 184 | `repo_map` | READ | yes | no |
| 185 | `context_expand` | READ | yes | no |
| 186 | `recovery_status` | READ | yes | no |
| 187 | `tool_schema_list` | READ | yes | no |
| 188 | `tool_schema_register` | WRITE | no | no |
| 189 | `capabilities` | READ | yes | no |
| 190 | `tool_search` | READ | yes | no |
| 191 | `tool_dynamic_filter` | READ | yes | no |
| 192 | `tool_describe` | READ | yes | no |
| 193 | `tool_categories` | READ | yes | no |
| 194 | `tool_function_find` | READ | yes | no |
| 195 | `tool_aliases` | READ | yes | no |
| 196 | `mcp_hub` | READ | yes | no |
| 197 | `dev_context` | READ | yes | no |
| 198 | `recipe_catalog` | READ | yes | no |
| 199 | `capture_screenshot` | READ | yes | no |
| 200 | `compare_screenshot` | READ | yes | no |
| 201 | `dom_snapshot` | READ | yes | no |
| 202 | `layout_metadata` | READ | yes | no |
| 203 | `visual_context` | READ | yes | no |
| 204 | `inspect_pdf` | READ | yes | no |
| 205 | `compare_pdf_pages` | READ | yes | no |
| 206 | `project_profile_get` | READ | yes | no |
| 207 | `project_profile_set` | WRITE | no | no |
| 208 | `handoff_context` | READ | yes | no |
| 209 | `benchmark_run` | EXECUTE | no | no |
| 210 | `regression_report` | READ | yes | no |
| 211 | `lsp_diagnostics` | READ | yes | no |
| 212 | `lsp_rename` | WRITE | no | no |
| 213 | `debug_attach` | EXECUTE | no | no |
| 214 | `debug_step` | EXECUTE | no | no |
| 215 | `git_worktree_spawn` | DANGEROUS | no | yes |
| 216 | `git_worktree_remove` | DANGEROUS | no | yes |
| 217 | `pdf_extract_tables` | READ | yes | no |
| 218 | `self_heal_plan` | READ | yes | no |
| 219 | `self_heal_apply` | DANGEROUS | no | yes |
| 220 | `skills_import` | WRITE | no | no |
| 221 | `agent_swarm_run` | EXECUTE | no | no |
| 222 | `policy_explain` | READ | yes | no |

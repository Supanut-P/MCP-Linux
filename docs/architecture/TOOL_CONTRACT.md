...
This block is generated from the built `ToolRegistry`. Current count: **204 tools**.
...
| 58 | `skills_list` | DANGEROUS | no | yes |
| 59 | `skills_read` | DANGEROUS | no | yes |
| 60 | `mcp_list` | DANGEROUS | no | yes |
| 61 | `mcp_describe` | DANGEROUS | no | yes |
| 62 | `mcp_call` | DANGEROUS | no | yes |
| 63 | `workspace_context` | READ | yes | no |
| 64 | `workspace_context_continue` | READ | yes | no |
| 65 | `workspace_full_scan` | READ | yes | no |
| 66 | `workspace_full_scan_continue` | READ | yes | no |
| 67 | `workspace_snapshot` | READ | yes | no |
| 68 | `search_all` | READ | yes | no |
| 69 | `read_many_files` | READ | yes | no |
| 70 | `read_file_page` | READ | yes | no |
| 71 | `read_file_page_continue` | READ | yes | no |
| 72 | `workspace_index` | READ | yes | no |
| 73 | `workspace_index_status` | READ | yes | no |
| 74 | `workspace_index_watch` | READ | yes | no |
| 75 | `workspace_index_stop` | READ | yes | no |
| 76 | `session_handoff` | READ | yes | no |
| 77 | `verify_incremental` | EXECUTE | no | no |
| 78 | `symbol_search` | READ | yes | no |
| 79 | `find_definition` | READ | yes | no |
| 80 | `find_references` | READ | yes | no |
| 81 | `find_implementations` | READ | yes | no |
| 82 | `call_hierarchy` | READ | yes | no |
| 83 | `import_graph` | READ | yes | no |
| 84 | `dependency_graph` | READ | yes | no |
| 85 | `module_graph` | READ | yes | no |
| 86 | `type_search` | READ | yes | no |
| 87 | `trace_symbol` | READ | yes | no |
| 88 | `context_ranking` | READ | yes | no |
| 89 | `debug_context` | READ | yes | no |
| 90 | `review_context` | READ | yes | no |
| 91 | `change_context` | READ | yes | no |
| 92 | `symbol_context` | READ | yes | no |
| 93 | `test_context` | READ | yes | no |
| 94 | `dependency_context` | READ | yes | no |
| 95 | `git_context` | READ | yes | no |
| 96 | `frontend_context` | READ | yes | no |
| 97 | `backend_context` | READ | yes | no |
| 98 | `route_intent` | READ | yes | no |
| 99 | `recipe_list` | READ | yes | no |
| 100 | `recipe_describe` | READ | yes | no |
| 101 | `recipe_run` | EXECUTE | no | no |
| 102 | `dry_run` | READ | yes | no |
| 103 | `review_changes` | READ | yes | no |
| 104 | `changed_symbols` | READ | yes | no |
| 105 | `affected_modules` | READ | yes | no |
| 106 | `git_history_context` | READ | yes | no |
| 107 | `git_blame_context` | READ | yes | no |
| 108 | `discover_tests` | READ | yes | no |
| 109 | `run_affected_tests` | EXECUTE | no | no |
| 110 | `test_failures` | READ | yes | no |
| 111 | `coverage_context` | READ | yes | no |
| 112 | `test_history` | READ | yes | no |
| 113 | `cache_stats` | READ | yes | no |
| 114 | `cache_clear` | WRITE | no | no |
| 115 | `cache_invalidate` | WRITE | no | no |
| 116 | `hook_list` | READ | yes | no |
| 117 | `hook_register` | WRITE | no | no |
| 118 | `hook_remove` | WRITE | no | no |
| 119 | `skill_match` | READ | yes | no |
| 120 | `skill_load` | READ | yes | no |
| 121 | `plugin_install` | DANGEROUS | no | yes |
| 122 | `plugin_list` | READ | yes | no |
| 123 | `plugin_enable` | WRITE | no | no |
| 124 | `plugin_disable` | WRITE | no | no |
| 125 | `plugin_remove` | DANGEROUS | no | yes |
| 126 | `session_context` | READ | yes | no |
| 127 | `session_checkpoint` | WRITE | no | no |
| 128 | `session_resume` | READ | yes | no |
| 129 | `session_history` | READ | yes | no |
| 130 | `response_mode` | READ | yes | no |
| 131 | `inspect_web_app` | READ | yes | no |
| 132 | `debug_ui` | READ | yes | no |
| 133 | `capture_ui_state` | READ | yes | no |
| 134 | `form_context` | READ | yes | no |
| 135 | `network_context` | READ | yes | no |
| 136 | `console_context` | READ | yes | no |
| 137 | `browser_debug_context` | READ | yes | no |
| 138 | `service_context` | READ | yes | no |
| 139 | `process_context` | READ | yes | no |
| 140 | `port_context` | READ | yes | no |
...
| 184 | `inspect_pdf` | READ | yes | no |
| 185 | `compare_pdf_pages` | READ | yes | no |
| 186 | `project_profile_get` | READ | yes | no |
| 187 | `project_profile_set` | WRITE | no | no |
| 188 | `handoff_context` | READ | yes | no |
| 189 | `benchmark_run` | EXECUTE | no | no |
| 190 | `regression_report` | READ | yes | no |
| 191 | `lsp_diagnostics` | READ | yes | no |
| 192 | `lsp_rename` | WRITE | no | no |
| 193 | `debug_attach` | EXECUTE | no | no |
| 194 | `debug_step` | EXECUTE | no | no |
| 195 | `git_worktree_spawn` | DANGEROUS | no | yes |
| 196 | `git_worktree_remove` | DANGEROUS | no | yes |
| 197 | `db_inspect` | READ | yes | no |
| 198 | `db_query` | DANGEROUS | no | yes |
| 199 | `pdf_extract_tables` | READ | yes | no |
| 200 | `self_heal_plan` | READ | yes | no |
| 201 | `self_heal_apply` | DANGEROUS | no | yes |
| 202 | `skills_import` | WRITE | no | no |
| 203 | `agent_swarm_run` | EXECUTE | no | no |
| 204 | `tool_batch` | DANGEROUS | no | yes |

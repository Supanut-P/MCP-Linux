...
This block is generated from the built `ToolRegistry`. Current count: **196 tools**.
...
| 50 | `skills_list` | DANGEROUS | no | yes |
| 51 | `skills_read` | DANGEROUS | no | yes |
| 52 | `mcp_list` | DANGEROUS | no | yes |
| 53 | `mcp_describe` | DANGEROUS | no | yes |
| 54 | `mcp_call` | DANGEROUS | no | yes |
| 55 | `workspace_context` | READ | yes | no |
| 56 | `workspace_context_continue` | READ | yes | no |
| 57 | `workspace_full_scan` | READ | yes | no |
| 58 | `workspace_full_scan_continue` | READ | yes | no |
| 59 | `workspace_snapshot` | READ | yes | no |
| 60 | `search_all` | READ | yes | no |
| 61 | `read_many_files` | READ | yes | no |
| 62 | `read_file_page` | READ | yes | no |
| 63 | `read_file_page_continue` | READ | yes | no |
| 64 | `workspace_index` | READ | yes | no |
| 65 | `workspace_index_status` | READ | yes | no |
| 66 | `workspace_index_watch` | READ | yes | no |
| 67 | `workspace_index_stop` | READ | yes | no |
| 68 | `session_handoff` | READ | yes | no |
| 69 | `verify_incremental` | EXECUTE | no | no |
| 70 | `symbol_search` | READ | yes | no |
| 71 | `find_definition` | READ | yes | no |
| 72 | `find_references` | READ | yes | no |
| 73 | `find_implementations` | READ | yes | no |
| 74 | `call_hierarchy` | READ | yes | no |
| 75 | `import_graph` | READ | yes | no |
| 76 | `dependency_graph` | READ | yes | no |
| 77 | `module_graph` | READ | yes | no |
| 78 | `type_search` | READ | yes | no |
| 79 | `trace_symbol` | READ | yes | no |
| 80 | `context_ranking` | READ | yes | no |
| 81 | `debug_context` | READ | yes | no |
| 82 | `review_context` | READ | yes | no |
| 83 | `change_context` | READ | yes | no |
| 84 | `symbol_context` | READ | yes | no |
| 85 | `test_context` | READ | yes | no |
| 86 | `dependency_context` | READ | yes | no |
| 87 | `git_context` | READ | yes | no |
| 88 | `frontend_context` | READ | yes | no |
| 89 | `backend_context` | READ | yes | no |
| 90 | `route_intent` | READ | yes | no |
| 91 | `recipe_list` | READ | yes | no |
| 92 | `recipe_describe` | READ | yes | no |
| 93 | `recipe_run` | EXECUTE | no | no |
| 94 | `dry_run` | READ | yes | no |
| 95 | `review_changes` | READ | yes | no |
| 96 | `changed_symbols` | READ | yes | no |
| 97 | `affected_modules` | READ | yes | no |
| 98 | `git_history_context` | READ | yes | no |
| 99 | `git_blame_context` | READ | yes | no |
| 100 | `discover_tests` | READ | yes | no |
| 101 | `run_affected_tests` | EXECUTE | no | no |
| 102 | `test_failures` | READ | yes | no |
| 103 | `coverage_context` | READ | yes | no |
| 104 | `test_history` | READ | yes | no |
| 105 | `cache_stats` | READ | yes | no |
| 106 | `cache_clear` | WRITE | no | no |
| 107 | `cache_invalidate` | WRITE | no | no |
| 108 | `hook_list` | READ | yes | no |
| 109 | `hook_register` | WRITE | no | no |
| 110 | `hook_remove` | WRITE | no | no |
| 111 | `skill_match` | READ | yes | no |
| 112 | `skill_load` | READ | yes | no |
| 113 | `plugin_install` | DANGEROUS | no | yes |
| 114 | `plugin_list` | READ | yes | no |
| 115 | `plugin_enable` | WRITE | no | no |
| 116 | `plugin_disable` | WRITE | no | no |
| 117 | `plugin_remove` | DANGEROUS | no | yes |
| 118 | `session_context` | READ | yes | no |
| 119 | `session_checkpoint` | WRITE | no | no |
| 120 | `session_resume` | READ | yes | no |
| 121 | `session_history` | READ | yes | no |
| 122 | `response_mode` | READ | yes | no |
| 123 | `inspect_web_app` | READ | yes | no |
| 124 | `debug_ui` | READ | yes | no |
| 125 | `capture_ui_state` | READ | yes | no |
| 126 | `form_context` | READ | yes | no |
| 127 | `network_context` | READ | yes | no |
| 128 | `console_context` | READ | yes | no |
| 129 | `browser_debug_context` | READ | yes | no |
| 130 | `service_context` | READ | yes | no |
| 131 | `process_context` | READ | yes | no |
| 132 | `port_context` | READ | yes | no |
| 133 | `installed_runtime_context` | READ | yes | no |
| 134 | `path_context` | READ | yes | no |
| 135 | `startup_context` | READ | yes | no |
| 136 | `mcp_discover` | READ | yes | no |
| 137 | `mcp_health` | READ | yes | no |
| 138 | `mcp_resources` | READ | yes | no |
| 139 | `task_create` | EXECUTE | no | no |
| 140 | `task_status` | READ | yes | no |
| 141 | `task_cancel` | EXECUTE | no | no |
| 142 | `task_result` | READ | yes | no |
| 143 | `task_list` | READ | yes | no |
| 144 | `delegate` | EXECUTE | no | no |
| 145 | `delegate_status` | READ | yes | no |
| 146 | `delegate_cancel` | EXECUTE | no | no |
| 147 | `delegate_result` | READ | yes | no |
| 148 | `parallel_delegate` | EXECUTE | no | no |
| 149 | `permission_check` | READ | yes | no |
| 150 | `permission_profile` | READ | yes | no |
| 151 | `live_logs_query` | READ | yes | no |
| 152 | `live_logs_status` | READ | yes | no |
| 153 | `telemetry_dashboard` | READ | yes | no |
| 154 | `context_economy_stats` | READ | yes | no |
| 155 | `execution_plan` | READ | yes | no |
| 156 | `repo_map` | READ | yes | no |
| 157 | `context_expand` | READ | yes | no |
| 158 | `recovery_status` | READ | yes | no |
| 159 | `tool_schema_list` | READ | yes | no |
| 160 | `tool_schema_register` | WRITE | no | no |
| 161 | `capabilities` | READ | yes | no |
| 162 | `tool_search` | READ | yes | no |
| 163 | `tool_dynamic_filter` | READ | yes | no |
| 164 | `tool_describe` | READ | yes | no |
| 165 | `tool_categories` | READ | yes | no |
| 166 | `tool_function_find` | READ | yes | no |
| 167 | `tool_aliases` | READ | yes | no |
| 168 | `mcp_hub` | READ | yes | no |
| 169 | `dev_context` | READ | yes | no |
| 170 | `recipe_catalog` | READ | yes | no |
| 171 | `capture_screenshot` | READ | yes | no |
| 172 | `compare_screenshot` | READ | yes | no |
| 173 | `dom_snapshot` | READ | yes | no |
| 174 | `layout_metadata` | READ | yes | no |
| 175 | `visual_context` | READ | yes | no |
| 176 | `inspect_pdf` | READ | yes | no |
| 177 | `compare_pdf_pages` | READ | yes | no |
| 178 | `project_profile_get` | READ | yes | no |
| 179 | `project_profile_set` | WRITE | no | no |
| 180 | `handoff_context` | READ | yes | no |
| 181 | `benchmark_run` | EXECUTE | no | no |
| 182 | `regression_report` | READ | yes | no |
| 183 | `lsp_diagnostics` | READ | yes | no |
| 184 | `lsp_rename` | WRITE | no | no |
| 185 | `debug_attach` | EXECUTE | no | no |
| 186 | `debug_step` | EXECUTE | no | no |
| 187 | `git_worktree_spawn` | DANGEROUS | no | yes |
| 188 | `git_worktree_remove` | DANGEROUS | no | yes |
| 189 | `db_inspect` | READ | yes | no |
| 190 | `db_query` | DANGEROUS | no | yes |
| 191 | `pdf_extract_tables` | READ | yes | no |
| 192 | `self_heal_plan` | READ | yes | no |
| 193 | `self_heal_apply` | DANGEROUS | no | yes |
| 194 | `skills_import` | WRITE | no | no |
| 195 | `agent_swarm_run` | EXECUTE | no | no |
| 196 | `tool_batch` | DANGEROUS | no | yes |

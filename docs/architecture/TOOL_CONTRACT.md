...
This block is generated from the built `ToolRegistry`. Current count: **201 tools**.
...
| 135 | `service_context` | READ | yes | no |
| 136 | `process_context` | READ | yes | no |
| 137 | `port_context` | READ | yes | no |
| 138 | `installed_runtime_context` | READ | yes | no |
| 139 | `path_context` | READ | yes | no |
| 140 | `startup_context` | READ | yes | no |
| 141 | `mcp_discover` | READ | yes | no |
| 142 | `mcp_health` | READ | yes | no |
| 143 | `mcp_resources` | READ | yes | no |
| 144 | `task_create` | EXECUTE | no | no |
| 145 | `task_status` | READ | yes | no |
| 146 | `task_cancel` | EXECUTE | no | no |
| 147 | `task_result` | READ | yes | no |
| 148 | `task_list` | READ | yes | no |
| 149 | `delegate` | EXECUTE | no | no |
| 150 | `delegate_status` | READ | yes | no |
| 151 | `delegate_cancel` | EXECUTE | no | no |
| 152 | `delegate_result` | READ | yes | no |
| 153 | `parallel_delegate` | EXECUTE | no | no |
| 154 | `permission_check` | READ | yes | no |
| 155 | `permission_profile` | READ | yes | no |
| 156 | `live_logs_query` | READ | yes | no |
| 157 | `live_logs_status` | READ | yes | no |
| 158 | `telemetry_dashboard` | READ | yes | no |
| 159 | `context_economy_stats` | READ | yes | no |
| 160 | `execution_plan` | READ | yes | no |
| 161 | `repo_map` | READ | yes | no |
| 162 | `context_expand` | READ | yes | no |
| 163 | `recovery_status` | READ | yes | no |
| 164 | `tool_schema_list` | READ | yes | no |
| 165 | `tool_schema_register` | WRITE | no | no |
| 166 | `capabilities` | READ | yes | no |
| 167 | `tool_search` | READ | yes | no |
| 168 | `tool_dynamic_filter` | READ | yes | no |
| 169 | `tool_describe` | READ | yes | no |
| 170 | `tool_categories` | READ | yes | no |
| 171 | `tool_function_find` | READ | yes | no |
| 172 | `tool_aliases` | READ | yes | no |
| 173 | `mcp_hub` | READ | yes | no |
| 174 | `dev_context` | READ | yes | no |
| 175 | `recipe_catalog` | READ | yes | no |
| 176 | `capture_screenshot` | READ | yes | no |
| 177 | `compare_screenshot` | READ | yes | no |
| 178 | `dom_snapshot` | READ | yes | no |
| 179 | `layout_metadata` | READ | yes | no |
| 180 | `visual_context` | READ | yes | no |
| 181 | `inspect_pdf` | READ | yes | no |
| 182 | `compare_pdf_pages` | READ | yes | no |
| 183 | `project_profile_get` | READ | yes | no |
| 184 | `project_profile_set` | WRITE | no | no |
| 185 | `handoff_context` | READ | yes | no |
| 186 | `benchmark_run` | EXECUTE | no | no |
| 187 | `regression_report` | READ | yes | no |
| 188 | `lsp_diagnostics` | READ | yes | no |
| 189 | `lsp_rename` | WRITE | no | no |
| 190 | `debug_attach` | EXECUTE | no | no |
| 191 | `debug_step` | EXECUTE | no | no |
| 192 | `git_worktree_spawn` | DANGEROUS | no | yes |
| 193 | `git_worktree_remove` | DANGEROUS | no | yes |
| 194 | `db_inspect` | READ | yes | no |
| 195 | `db_query` | DANGEROUS | no | yes |
| 196 | `pdf_extract_tables` | READ | yes | no |
| 197 | `self_heal_plan` | READ | yes | no |
| 198 | `self_heal_apply` | DANGEROUS | no | yes |
| 199 | `skills_import` | WRITE | no | no |
| 200 | `agent_swarm_run` | EXECUTE | no | no |
| 201 | `tool_batch` | DANGEROUS | no | yes |

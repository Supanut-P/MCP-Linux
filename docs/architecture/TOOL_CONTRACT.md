...
This block is generated from the built `ToolRegistry`. Current count: **211 tools**.
...
| 66 | `target_catalog` | READ | yes | no |
| 67 | `skills_list` | DANGEROUS | no | yes |
| 68 | `skills_read` | DANGEROUS | no | yes |
| 69 | `mcp_list` | DANGEROUS | no | yes |
| 70 | `mcp_describe` | DANGEROUS | no | yes |
| 71 | `mcp_call` | DANGEROUS | no | yes |
| 72 | `workspace_context` | READ | yes | no |
| 73 | `workspace_context_continue` | READ | yes | no |
| 74 | `workspace_full_scan` | READ | yes | no |
| 75 | `workspace_full_scan_continue` | READ | yes | no |
| 76 | `workspace_snapshot` | READ | yes | no |
| 77 | `search_all` | READ | yes | no |
| 78 | `read_many_files` | READ | yes | no |
| 79 | `read_file_page` | READ | yes | no |
| 80 | `read_file_page_continue` | READ | yes | no |
| 81 | `workspace_index` | READ | yes | no |
| 82 | `workspace_index_status` | READ | yes | no |
| 83 | `workspace_index_watch` | READ | yes | no |
| 84 | `workspace_index_stop` | READ | yes | no |
| 85 | `session_handoff` | READ | yes | no |
| 86 | `verify_incremental` | EXECUTE | no | no |
| 87 | `symbol_search` | READ | yes | no |
| 88 | `find_definition` | READ | yes | no |
| 89 | `find_references` | READ | yes | no |
| 90 | `find_implementations` | READ | yes | no |
| 91 | `call_hierarchy` | READ | yes | no |
| 92 | `import_graph` | READ | yes | no |
| 93 | `dependency_graph` | READ | yes | no |
| 94 | `module_graph` | READ | yes | no |
| 95 | `type_search` | READ | yes | no |
| 96 | `trace_symbol` | READ | yes | no |
| 97 | `context_ranking` | READ | yes | no |
| 98 | `debug_context` | READ | yes | no |
| 99 | `review_context` | READ | yes | no |
| 100 | `change_context` | READ | yes | no |
| 101 | `symbol_context` | READ | yes | no |
| 102 | `test_context` | READ | yes | no |
| 103 | `dependency_context` | READ | yes | no |
| 104 | `git_context` | READ | yes | no |
| 105 | `frontend_context` | READ | yes | no |
| 106 | `backend_context` | READ | yes | no |
| 107 | `route_intent` | READ | yes | no |
| 108 | `recipe_list` | READ | yes | no |
| 109 | `recipe_describe` | READ | yes | no |
| 110 | `recipe_run` | EXECUTE | no | no |
| 111 | `dry_run` | READ | yes | no |
| 112 | `review_changes` | READ | yes | no |
| 113 | `changed_symbols` | READ | yes | no |
| 114 | `affected_modules` | READ | yes | no |
| 115 | `git_history_context` | READ | yes | no |
| 116 | `git_blame_context` | READ | yes | no |
| 117 | `discover_tests` | READ | yes | no |
| 118 | `run_affected_tests` | EXECUTE | no | no |
| 119 | `test_failures` | READ | yes | no |
| 120 | `coverage_context` | READ | yes | no |
| 121 | `test_history` | READ | yes | no |
| 122 | `cache_stats` | READ | yes | no |
| 123 | `cache_clear` | WRITE | no | no |
| 124 | `cache_invalidate` | WRITE | no | no |
| 125 | `hook_list` | READ | yes | no |
| 126 | `hook_register` | WRITE | no | no |
| 127 | `hook_remove` | WRITE | no | no |
| 128 | `skill_match` | READ | yes | no |
| 129 | `skill_load` | READ | yes | no |
| 130 | `plugin_install` | DANGEROUS | no | yes |
| 131 | `plugin_list` | READ | yes | no |
| 132 | `plugin_enable` | WRITE | no | no |
| 133 | `plugin_disable` | WRITE | no | no |
| 134 | `plugin_remove` | DANGEROUS | no | yes |
| 135 | `session_context` | READ | yes | no |
| 136 | `session_checkpoint` | WRITE | no | no |
| 137 | `session_resume` | READ | yes | no |
| 138 | `session_history` | READ | yes | no |
| 139 | `response_mode` | READ | yes | no |
| 140 | `inspect_web_app` | READ | yes | no |
| 141 | `debug_ui` | READ | yes | no |
| 142 | `capture_ui_state` | READ | yes | no |
| 143 | `form_context` | READ | yes | no |
| 144 | `network_context` | READ | yes | no |
| 145 | `console_context` | READ | yes | no |
| 146 | `browser_debug_context` | READ | yes | no |
| 147 | `service_context` | READ | yes | no |
| 148 | `process_context` | READ | yes | no |
| 149 | `port_context` | READ | yes | no |
| 150 | `installed_runtime_context` | READ | yes | no |
| 151 | `path_context` | READ | yes | no |
| 152 | `startup_context` | READ | yes | no |
| 153 | `mcp_discover` | READ | yes | no |
| 154 | `mcp_health` | READ | yes | no |
| 155 | `mcp_resources` | READ | yes | no |
| 156 | `task_create` | EXECUTE | no | no |
| 157 | `task_status` | READ | yes | no |
| 158 | `task_cancel` | EXECUTE | no | no |
| 159 | `task_result` | READ | yes | no |
| 160 | `task_list` | READ | yes | no |
| 161 | `delegate` | EXECUTE | no | no |
| 162 | `delegate_status` | READ | yes | no |
| 163 | `delegate_cancel` | EXECUTE | no | no |
| 164 | `delegate_result` | READ | yes | no |
| 165 | `parallel_delegate` | EXECUTE | no | no |
| 166 | `permission_check` | READ | yes | no |
| 167 | `permission_profile` | READ | yes | no |
| 168 | `live_logs_query` | READ | yes | no |
| 169 | `live_logs_status` | READ | yes | no |
| 170 | `telemetry_dashboard` | READ | yes | no |
| 171 | `context_economy_stats` | READ | yes | no |
| 172 | `execution_plan` | READ | yes | no |
| 173 | `repo_map` | READ | yes | no |
| 174 | `context_expand` | READ | yes | no |
| 175 | `recovery_status` | READ | yes | no |
| 176 | `tool_schema_list` | READ | yes | no |
| 177 | `tool_schema_register` | WRITE | no | no |
| 178 | `capabilities` | READ | yes | no |
| 179 | `tool_search` | READ | yes | no |
| 180 | `tool_dynamic_filter` | READ | yes | no |
| 181 | `tool_describe` | READ | yes | no |
| 182 | `tool_categories` | READ | yes | no |
| 183 | `tool_function_find` | READ | yes | no |
| 184 | `tool_aliases` | READ | yes | no |
| 185 | `mcp_hub` | READ | yes | no |
| 186 | `dev_context` | READ | yes | no |
| 187 | `recipe_catalog` | READ | yes | no |
| 188 | `capture_screenshot` | READ | yes | no |
| 189 | `compare_screenshot` | READ | yes | no |
| 190 | `dom_snapshot` | READ | yes | no |
| 191 | `layout_metadata` | READ | yes | no |
| 192 | `visual_context` | READ | yes | no |
| 193 | `inspect_pdf` | READ | yes | no |
| 194 | `compare_pdf_pages` | READ | yes | no |
| 195 | `project_profile_get` | READ | yes | no |
| 196 | `project_profile_set` | WRITE | no | no |
| 197 | `handoff_context` | READ | yes | no |
| 198 | `benchmark_run` | EXECUTE | no | no |
| 199 | `regression_report` | READ | yes | no |
| 200 | `lsp_diagnostics` | READ | yes | no |
| 201 | `lsp_rename` | WRITE | no | no |
| 202 | `debug_attach` | EXECUTE | no | no |
| 203 | `debug_step` | EXECUTE | no | no |
| 204 | `git_worktree_spawn` | DANGEROUS | no | yes |
| 205 | `git_worktree_remove` | DANGEROUS | no | yes |
| 206 | `pdf_extract_tables` | READ | yes | no |
| 207 | `self_heal_plan` | READ | yes | no |
| 208 | `self_heal_apply` | DANGEROUS | no | yes |
| 209 | `skills_import` | WRITE | no | no |
| 210 | `agent_swarm_run` | EXECUTE | no | no |
| 211 | `tool_batch` | DANGEROUS | no | yes |

...
| 70 | `db_inspect` | READ | yes | no |
| 71 | `db_query` | READ | yes | no |
| 72 | `target_catalog` | READ | yes | no |
| 73 | `support_bundle` | DANGEROUS | no | yes |
| 74 | `skills_list` | DANGEROUS | no | yes |
| 75 | `skills_read` | DANGEROUS | no | yes |
| 76 | `mcp_list` | DANGEROUS | no | yes |
| 77 | `mcp_describe` | DANGEROUS | no | yes |
| 78 | `mcp_call` | DANGEROUS | no | yes |
| 79 | `workspace_context` | READ | yes | no |
| 80 | `workspace_context_continue` | READ | yes | no |
| 81 | `workspace_full_scan` | READ | yes | no |
| 82 | `workspace_full_scan_continue` | READ | yes | no |
| 83 | `workspace_snapshot` | READ | yes | no |
| 84 | `search_all` | READ | yes | no |
| 85 | `read_many_files` | READ | yes | no |
| 86 | `read_file_page` | READ | yes | no |
| 87 | `read_file_page_continue` | READ | yes | no |
| 88 | `workspace_index` | READ | yes | no |
| 89 | `workspace_index_status` | READ | yes | no |
| 90 | `workspace_index_watch` | READ | yes | no |
| 91 | `workspace_index_stop` | READ | yes | no |
| 92 | `workspace_changes` | READ | yes | no |
| 93 | `session_handoff` | READ | yes | no |
| 94 | `verify_incremental` | EXECUTE | no | no |
| 95 | `symbol_search` | READ | yes | no |
| 96 | `find_definition` | READ | yes | no |
| 97 | `find_references` | READ | yes | no |
| 98 | `find_implementations` | READ | yes | no |
| 99 | `call_hierarchy` | READ | yes | no |
| 100 | `import_graph` | READ | yes | no |
| 101 | `dependency_graph` | READ | yes | no |
| 102 | `module_graph` | READ | yes | no |
| 103 | `type_search` | READ | yes | no |
| 104 | `trace_symbol` | READ | yes | no |
| 105 | `context_ranking` | READ | yes | no |
| 106 | `debug_context` | READ | yes | no |
| 107 | `review_context` | READ | yes | no |
| 108 | `change_context` | READ | yes | no |
| 109 | `symbol_context` | READ | yes | no |
| 110 | `test_context` | READ | yes | no |
| 111 | `dependency_context` | READ | yes | no |
| 112 | `git_context` | READ | yes | no |
| 113 | `frontend_context` | READ | yes | no |
| 114 | `backend_context` | READ | yes | no |
| 115 | `route_intent` | READ | yes | no |
| 116 | `recipe_list` | READ | yes | no |
| 117 | `recipe_describe` | READ | yes | no |
| 118 | `recipe_run` | EXECUTE | no | no |
| 119 | `dry_run` | READ | yes | no |
| 120 | `review_changes` | READ | yes | no |
| 121 | `changed_symbols` | READ | yes | no |
| 122 | `affected_modules` | READ | yes | no |
| 123 | `git_history_context` | READ | yes | no |
| 124 | `git_blame_context` | READ | yes | no |
| 125 | `discover_tests` | READ | yes | no |
| 126 | `run_affected_tests` | EXECUTE | no | no |
| 127 | `test_failures` | READ | yes | no |
| 128 | `coverage_context` | READ | yes | no |
| 129 | `test_history` | READ | yes | no |
| 130 | `cache_stats` | READ | yes | no |
| 131 | `cache_clear` | WRITE | no | no |
| 132 | `cache_invalidate` | WRITE | no | no |
| 133 | `hook_list` | READ | yes | no |
| 134 | `hook_register` | WRITE | no | no |
| 135 | `hook_remove` | WRITE | no | no |
| 136 | `skill_match` | READ | yes | no |
| 137 | `skill_load` | READ | yes | no |
| 138 | `plugin_install` | DANGEROUS | no | yes |
| 139 | `plugin_list` | READ | yes | no |
| 140 | `plugin_enable` | WRITE | no | no |
| 141 | `plugin_disable` | WRITE | no | no |
| 142 | `plugin_remove` | DANGEROUS | no | yes |
| 143 | `session_context` | READ | yes | no |
| 144 | `session_checkpoint` | WRITE | no | no |
| 145 | `session_resume` | READ | yes | no |
| 146 | `session_history` | READ | yes | no |
| 147 | `response_mode` | READ | yes | no |
| 148 | `inspect_web_app` | READ | yes | no |
| 149 | `debug_ui` | READ | yes | no |
| 150 | `capture_ui_state` | READ | yes | no |
| 151 | `form_context` | READ | yes | no |
| 152 | `network_context` | READ | yes | no |
| 153 | `console_context` | READ | yes | no |
| 154 | `browser_debug_context` | READ | yes | no |
| 155 | `service_context` | READ | yes | no |
| 156 | `process_context` | READ | yes | no |
| 157 | `port_context` | READ | yes | no |
| 158 | `installed_runtime_context` | READ | yes | no |
| 159 | `path_context` | READ | yes | no |
| 160 | `startup_context` | READ | yes | no |
| 161 | `mcp_discover` | READ | yes | no |
| 162 | `mcp_health` | READ | yes | no |
| 163 | `mcp_resources` | READ | yes | no |
| 164 | `task_create` | EXECUTE | no | no |
| 165 | `task_status` | READ | yes | no |
| 166 | `task_cancel` | EXECUTE | no | no |
| 167 | `task_result` | READ | yes | no |
| 168 | `task_list` | READ | yes | no |
| 169 | `delegate` | EXECUTE | no | no |
| 170 | `delegate_status` | READ | yes | no |
| 171 | `delegate_cancel` | EXECUTE | no | no |
| 172 | `delegate_result` | READ | yes | no |
| 173 | `parallel_delegate` | EXECUTE | no | no |
| 174 | `permission_check` | READ | yes | no |
| 175 | `permission_profile` | READ | yes | no |
| 176 | `live_logs_query` | READ | yes | no |
| 177 | `live_logs_status` | READ | yes | no |
| 178 | `telemetry_dashboard` | READ | yes | no |
| 179 | `context_economy_stats` | READ | yes | no |
| 180 | `execution_plan` | READ | yes | no |
| 181 | `repo_map` | READ | yes | no |
| 182 | `context_expand` | READ | yes | no |
| 183 | `recovery_status` | READ | yes | no |
| 184 | `tool_schema_list` | READ | yes | no |
| 185 | `tool_schema_register` | WRITE | no | no |
| 186 | `capabilities` | READ | yes | no |
| 187 | `tool_search` | READ | yes | no |
| 188 | `tool_dynamic_filter` | READ | yes | no |
| 189 | `tool_describe` | READ | yes | no |
| 190 | `tool_categories` | READ | yes | no |
| 191 | `tool_function_find` | READ | yes | no |
| 192 | `tool_aliases` | READ | yes | no |
| 193 | `mcp_hub` | READ | yes | no |
| 194 | `dev_context` | READ | yes | no |
| 195 | `recipe_catalog` | READ | yes | no |
| 196 | `capture_screenshot` | READ | yes | no |
| 197 | `compare_screenshot` | READ | yes | no |
| 198 | `dom_snapshot` | READ | yes | no |
| 199 | `layout_metadata` | READ | yes | no |
| 200 | `visual_context` | READ | yes | no |
...
| 208 | `lsp_diagnostics` | READ | yes | no |
| 209 | `lsp_rename` | WRITE | no | no |
| 210 | `debug_attach` | EXECUTE | no | no |
| 211 | `debug_step` | EXECUTE | no | no |
| 212 | `git_worktree_spawn` | DANGEROUS | no | yes |
| 213 | `git_worktree_remove` | DANGEROUS | no | yes |
| 214 | `pdf_extract_tables` | READ | yes | no |
| 215 | `self_heal_plan` | READ | yes | no |

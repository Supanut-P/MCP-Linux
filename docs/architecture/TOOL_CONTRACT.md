...
| 61 | `remote_rollout_resume` | DANGEROUS | no | yes |
| 62 | `artifact_verify` | READ | yes | no |
| 63 | `http_probe` | READ | yes | no |
| 64 | `storage_usage` | READ | yes | no |
| 65 | `backup` | DANGEROUS | no | yes |
| 66 | `db_inspect` | READ | yes | no |
| 67 | `db_query` | READ | yes | no |
| 68 | `target_catalog` | READ | yes | no |
| 69 | `support_bundle` | DANGEROUS | no | yes |
| 70 | `skills_list` | DANGEROUS | no | yes |
| 71 | `skills_read` | DANGEROUS | no | yes |
| 72 | `mcp_list` | DANGEROUS | no | yes |
| 73 | `mcp_describe` | DANGEROUS | no | yes |
| 74 | `mcp_call` | DANGEROUS | no | yes |
| 75 | `workspace_context` | READ | yes | no |
| 76 | `workspace_context_continue` | READ | yes | no |
| 77 | `workspace_full_scan` | READ | yes | no |
| 78 | `workspace_full_scan_continue` | READ | yes | no |
| 79 | `workspace_snapshot` | READ | yes | no |
| 80 | `search_all` | READ | yes | no |
| 81 | `read_many_files` | READ | yes | no |
| 82 | `read_file_page` | READ | yes | no |
| 83 | `read_file_page_continue` | READ | yes | no |
| 84 | `workspace_index` | READ | yes | no |
| 85 | `workspace_index_status` | READ | yes | no |
| 86 | `workspace_index_watch` | READ | yes | no |
| 87 | `workspace_index_stop` | READ | yes | no |
| 88 | `session_handoff` | READ | yes | no |
| 89 | `verify_incremental` | EXECUTE | no | no |
...
| 133 | `plugin_install` | DANGEROUS | no | yes |
...
| 153 | `installed_runtime_context` | READ | yes | no |
| 154 | `path_context` | READ | yes | no |
| 155 | `startup_context` | READ | yes | no |
| 156 | `mcp_discover` | READ | yes | no |
| 157 | `mcp_health` | READ | yes | no |
| 158 | `mcp_resources` | READ | yes | no |
| 159 | `task_create` | EXECUTE | no | no |
| 160 | `task_status` | READ | yes | no |
| 161 | `task_cancel` | EXECUTE | no | no |
| 162 | `task_result` | READ | yes | no |
| 163 | `task_list` | READ | yes | no |
| 164 | `delegate` | EXECUTE | no | no |
| 165 | `delegate_status` | READ | yes | no |
| 166 | `delegate_cancel` | EXECUTE | no | no |
| 167 | `delegate_result` | READ | yes | no |
| 168 | `parallel_delegate` | EXECUTE | no | no |
| 169 | `permission_check` | READ | yes | no |
| 170 | `permission_profile` | READ | yes | no |
| 171 | `live_logs_query` | READ | yes | no |
| 172 | `live_logs_status` | READ | yes | no |
| 173 | `telemetry_dashboard` | READ | yes | no |
| 174 | `context_economy_stats` | READ | yes | no |
| 175 | `execution_plan` | READ | yes | no |
| 176 | `repo_map` | READ | yes | no |
| 177 | `context_expand` | READ | yes | no |
| 178 | `recovery_status` | READ | yes | no |
| 179 | `tool_schema_list` | READ | yes | no |
| 180 | `tool_schema_register` | WRITE | no | no |
| 181 | `capabilities` | READ | yes | no |
| 182 | `tool_search` | READ | yes | no |
| 183 | `tool_dynamic_filter` | READ | yes | no |
| 184 | `tool_describe` | READ | yes | no |
| 185 | `tool_categories` | READ | yes | no |
| 186 | `tool_function_find` | READ | yes | no |
| 187 | `tool_aliases` | READ | yes | no |
| 188 | `mcp_hub` | READ | yes | no |
| 189 | `dev_context` | READ | yes | no |
| 190 | `recipe_catalog` | READ | yes | no |
| 191 | `capture_screenshot` | READ | yes | no |
| 192 | `compare_screenshot` | READ | yes | no |
| 193 | `dom_snapshot` | READ | yes | no |
| 194 | `layout_metadata` | READ | yes | no |
| 195 | `visual_context` | READ | yes | no |
| 196 | `inspect_pdf` | READ | yes | no |
| 197 | `compare_pdf_pages` | READ | yes | no |
| 198 | `project_profile_get` | READ | yes | no |
| 199 | `project_profile_set` | WRITE | no | no |
| 200 | `handoff_context` | READ | yes | no |
| 201 | `benchmark_run` | EXECUTE | no | no |
| 202 | `regression_report` | READ | yes | no |
| 203 | `lsp_diagnostics` | READ | yes | no |
| 204 | `lsp_rename` | WRITE | no | no |
| 205 | `debug_attach` | EXECUTE | no | no |
| 206 | `debug_step` | EXECUTE | no | no |
| 207 | `git_worktree_spawn` | DANGEROUS | no | yes |
| 208 | `git_worktree_remove` | DANGEROUS | no | yes |
| 209 | `pdf_extract_tables` | READ | yes | no |

...
This block is generated from the built `ToolRegistry`. Current count: **225 tools**.
...
| 75 | `db_inspect` | READ | yes | no |
| 76 | `db_query` | READ | yes | no |
| 77 | `target_catalog` | READ | yes | no |
| 78 | `support_bundle` | DANGEROUS | no | yes |
| 79 | `skills_list` | DANGEROUS | no | yes |
| 80 | `skills_read` | DANGEROUS | no | yes |
| 81 | `mcp_list` | DANGEROUS | no | yes |
| 82 | `mcp_describe` | DANGEROUS | no | yes |
| 83 | `mcp_call` | DANGEROUS | no | yes |
| 84 | `workspace_context` | READ | yes | no |
| 85 | `workspace_context_continue` | READ | yes | no |
| 86 | `workspace_full_scan` | READ | yes | no |
| 87 | `workspace_full_scan_continue` | READ | yes | no |
| 88 | `workspace_snapshot` | READ | yes | no |
| 89 | `search_all` | READ | yes | no |
| 90 | `read_many_files` | READ | yes | no |
| 91 | `read_file_page` | READ | yes | no |
| 92 | `read_file_page_continue` | READ | yes | no |
| 93 | `workspace_index` | READ | yes | no |
| 94 | `workspace_index_status` | READ | yes | no |
| 95 | `workspace_index_watch` | READ | yes | no |
| 96 | `workspace_index_stop` | READ | yes | no |
| 97 | `workspace_changes` | READ | yes | no |
| 98 | `session_handoff` | READ | yes | no |
| 99 | `verify_incremental` | EXECUTE | no | no |
| 100 | `symbol_search` | READ | yes | no |
| 101 | `find_definition` | READ | yes | no |
| 102 | `find_references` | READ | yes | no |
| 103 | `find_implementations` | READ | yes | no |
| 104 | `call_hierarchy` | READ | yes | no |
| 105 | `import_graph` | READ | yes | no |
| 106 | `dependency_graph` | READ | yes | no |
| 107 | `module_graph` | READ | yes | no |
| 108 | `type_search` | READ | yes | no |
| 109 | `trace_symbol` | READ | yes | no |
| 110 | `context_ranking` | READ | yes | no |
| 111 | `debug_context` | READ | yes | no |
| 112 | `review_context` | READ | yes | no |
| 113 | `change_context` | READ | yes | no |
| 114 | `symbol_context` | READ | yes | no |
| 115 | `test_context` | READ | yes | no |
| 116 | `dependency_context` | READ | yes | no |
| 117 | `git_context` | READ | yes | no |
| 118 | `frontend_context` | READ | yes | no |
| 119 | `backend_context` | READ | yes | no |
| 120 | `route_intent` | READ | yes | no |
| 121 | `recipe_list` | READ | yes | no |
| 122 | `recipe_describe` | READ | yes | no |
| 123 | `recipe_run` | EXECUTE | no | no |
| 124 | `dry_run` | READ | yes | no |
| 125 | `review_changes` | READ | yes | no |
| 126 | `changed_symbols` | READ | yes | no |
| 127 | `affected_modules` | READ | yes | no |
| 128 | `git_history_context` | READ | yes | no |
| 129 | `git_blame_context` | READ | yes | no |
| 130 | `discover_tests` | READ | yes | no |
| 131 | `run_affected_tests` | EXECUTE | no | no |
| 132 | `test_failures` | READ | yes | no |
| 133 | `coverage_context` | READ | yes | no |
| 134 | `test_history` | READ | yes | no |
| 135 | `cache_stats` | READ | yes | no |
| 136 | `cache_clear` | WRITE | no | no |
| 137 | `cache_invalidate` | WRITE | no | no |
| 138 | `hook_list` | READ | yes | no |
| 139 | `hook_register` | WRITE | no | no |
| 140 | `hook_remove` | WRITE | no | no |
| 141 | `skill_match` | READ | yes | no |
| 142 | `skill_load` | READ | yes | no |
| 143 | `plugin_install` | DANGEROUS | no | yes |
| 144 | `plugin_list` | READ | yes | no |
| 145 | `plugin_enable` | WRITE | no | no |
| 146 | `plugin_disable` | WRITE | no | no |
| 147 | `plugin_remove` | DANGEROUS | no | yes |
| 148 | `session_context` | READ | yes | no |
| 149 | `session_checkpoint` | WRITE | no | no |
| 150 | `session_resume` | READ | yes | no |
| 151 | `session_history` | READ | yes | no |
| 152 | `response_mode` | READ | yes | no |
| 153 | `inspect_web_app` | READ | yes | no |
| 154 | `debug_ui` | READ | yes | no |
| 155 | `capture_ui_state` | READ | yes | no |
| 156 | `form_context` | READ | yes | no |
| 157 | `network_context` | READ | yes | no |
| 158 | `console_context` | READ | yes | no |
| 159 | `browser_debug_context` | READ | yes | no |
| 160 | `service_context` | READ | yes | no |
| 161 | `process_context` | READ | yes | no |
| 162 | `port_context` | READ | yes | no |
| 163 | `installed_runtime_context` | READ | yes | no |
| 164 | `path_context` | READ | yes | no |
| 165 | `startup_context` | READ | yes | no |
| 166 | `mcp_discover` | READ | yes | no |
| 167 | `mcp_health` | READ | yes | no |
| 168 | `mcp_resources` | READ | yes | no |
| 169 | `task_create` | EXECUTE | no | no |
| 170 | `task_status` | READ | yes | no |
| 171 | `task_cancel` | EXECUTE | no | no |
| 172 | `task_result` | READ | yes | no |
| 173 | `task_list` | READ | yes | no |
| 174 | `delegate` | EXECUTE | no | no |
| 175 | `delegate_status` | READ | yes | no |
| 176 | `delegate_cancel` | EXECUTE | no | no |
| 177 | `delegate_result` | READ | yes | no |
| 178 | `parallel_delegate` | EXECUTE | no | no |
| 179 | `permission_check` | READ | yes | no |
| 180 | `permission_profile` | READ | yes | no |
| 181 | `live_logs_query` | READ | yes | no |
| 182 | `live_logs_status` | READ | yes | no |
| 183 | `telemetry_dashboard` | READ | yes | no |
| 184 | `context_economy_stats` | READ | yes | no |
| 185 | `execution_plan` | READ | yes | no |
| 186 | `repo_map` | READ | yes | no |
| 187 | `context_expand` | READ | yes | no |
| 188 | `recovery_status` | READ | yes | no |
| 189 | `tool_schema_list` | READ | yes | no |
| 190 | `tool_schema_register` | WRITE | no | no |
| 191 | `capabilities` | READ | yes | no |
| 192 | `tool_search` | READ | yes | no |
| 193 | `tool_dynamic_filter` | READ | yes | no |
| 194 | `tool_describe` | READ | yes | no |
| 195 | `tool_categories` | READ | yes | no |
| 196 | `tool_function_find` | READ | yes | no |
| 197 | `tool_aliases` | READ | yes | no |
| 198 | `mcp_hub` | READ | yes | no |
| 199 | `dev_context` | READ | yes | no |
| 200 | `recipe_catalog` | READ | yes | no |
| 201 | `capture_screenshot` | READ | yes | no |
| 202 | `compare_screenshot` | READ | yes | no |
| 203 | `dom_snapshot` | READ | yes | no |
| 204 | `layout_metadata` | READ | yes | no |
| 205 | `visual_context` | READ | yes | no |
| 206 | `inspect_pdf` | READ | yes | no |
| 207 | `compare_pdf_pages` | READ | yes | no |
| 208 | `project_profile_get` | READ | yes | no |
| 209 | `project_profile_set` | WRITE | no | no |
| 210 | `handoff_context` | READ | yes | no |
| 211 | `benchmark_run` | EXECUTE | no | no |
| 212 | `regression_report` | READ | yes | no |
| 213 | `lsp_diagnostics` | READ | yes | no |
| 214 | `lsp_rename` | WRITE | no | no |
| 215 | `debug_attach` | EXECUTE | no | no |
| 216 | `debug_step` | EXECUTE | no | no |
| 217 | `git_worktree_spawn` | DANGEROUS | no | yes |
| 218 | `git_worktree_remove` | DANGEROUS | no | yes |
| 219 | `pdf_extract_tables` | READ | yes | no |
| 220 | `self_heal_plan` | READ | yes | no |
| 221 | `self_heal_apply` | DANGEROUS | no | yes |
| 222 | `skills_import` | WRITE | no | no |
| 223 | `agent_swarm_run` | EXECUTE | no | no |
| 224 | `policy_explain` | READ | yes | no |
| 225 | `tool_batch` | DANGEROUS | no | yes |

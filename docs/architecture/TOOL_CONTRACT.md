...
This block is generated from the built `ToolRegistry`. Current count: **224 tools**.
...
| 74 | `db_inspect` | READ | yes | no |
| 75 | `db_query` | READ | yes | no |
| 76 | `target_catalog` | READ | yes | no |
| 77 | `support_bundle` | DANGEROUS | no | yes |
| 78 | `skills_list` | DANGEROUS | no | yes |
| 79 | `skills_read` | DANGEROUS | no | yes |
| 80 | `mcp_list` | DANGEROUS | no | yes |
| 81 | `mcp_describe` | DANGEROUS | no | yes |
| 82 | `mcp_call` | DANGEROUS | no | yes |
| 83 | `workspace_context` | READ | yes | no |
| 84 | `workspace_context_continue` | READ | yes | no |
| 85 | `workspace_full_scan` | READ | yes | no |
| 86 | `workspace_full_scan_continue` | READ | yes | no |
| 87 | `workspace_snapshot` | READ | yes | no |
| 88 | `search_all` | READ | yes | no |
| 89 | `read_many_files` | READ | yes | no |
| 90 | `read_file_page` | READ | yes | no |
| 91 | `read_file_page_continue` | READ | yes | no |
| 92 | `workspace_index` | READ | yes | no |
| 93 | `workspace_index_status` | READ | yes | no |
| 94 | `workspace_index_watch` | READ | yes | no |
| 95 | `workspace_index_stop` | READ | yes | no |
| 96 | `workspace_changes` | READ | yes | no |
| 97 | `session_handoff` | READ | yes | no |
| 98 | `verify_incremental` | EXECUTE | no | no |
| 99 | `symbol_search` | READ | yes | no |
| 100 | `find_definition` | READ | yes | no |
| 101 | `find_references` | READ | yes | no |
| 102 | `find_implementations` | READ | yes | no |
| 103 | `call_hierarchy` | READ | yes | no |
| 104 | `import_graph` | READ | yes | no |
| 105 | `dependency_graph` | READ | yes | no |
| 106 | `module_graph` | READ | yes | no |
| 107 | `type_search` | READ | yes | no |
| 108 | `trace_symbol` | READ | yes | no |
| 109 | `context_ranking` | READ | yes | no |
| 110 | `debug_context` | READ | yes | no |
| 111 | `review_context` | READ | yes | no |
| 112 | `change_context` | READ | yes | no |
| 113 | `symbol_context` | READ | yes | no |
| 114 | `test_context` | READ | yes | no |
| 115 | `dependency_context` | READ | yes | no |
| 116 | `git_context` | READ | yes | no |
| 117 | `frontend_context` | READ | yes | no |
| 118 | `backend_context` | READ | yes | no |
| 119 | `route_intent` | READ | yes | no |
| 120 | `recipe_list` | READ | yes | no |
| 121 | `recipe_describe` | READ | yes | no |
| 122 | `recipe_run` | EXECUTE | no | no |
| 123 | `dry_run` | READ | yes | no |
| 124 | `review_changes` | READ | yes | no |
| 125 | `changed_symbols` | READ | yes | no |
| 126 | `affected_modules` | READ | yes | no |
| 127 | `git_history_context` | READ | yes | no |
| 128 | `git_blame_context` | READ | yes | no |
| 129 | `discover_tests` | READ | yes | no |
| 130 | `run_affected_tests` | EXECUTE | no | no |
| 131 | `test_failures` | READ | yes | no |
| 132 | `coverage_context` | READ | yes | no |
| 133 | `test_history` | READ | yes | no |
| 134 | `cache_stats` | READ | yes | no |
| 135 | `cache_clear` | WRITE | no | no |
| 136 | `cache_invalidate` | WRITE | no | no |
| 137 | `hook_list` | READ | yes | no |
| 138 | `hook_register` | WRITE | no | no |
| 139 | `hook_remove` | WRITE | no | no |
| 140 | `skill_match` | READ | yes | no |
| 141 | `skill_load` | READ | yes | no |
| 142 | `plugin_install` | DANGEROUS | no | yes |
| 143 | `plugin_list` | READ | yes | no |
| 144 | `plugin_enable` | WRITE | no | no |
| 145 | `plugin_disable` | WRITE | no | no |
| 146 | `plugin_remove` | DANGEROUS | no | yes |
| 147 | `session_context` | READ | yes | no |
| 148 | `session_checkpoint` | WRITE | no | no |
| 149 | `session_resume` | READ | yes | no |
| 150 | `session_history` | READ | yes | no |
| 151 | `response_mode` | READ | yes | no |
| 152 | `inspect_web_app` | READ | yes | no |
| 153 | `debug_ui` | READ | yes | no |
| 154 | `capture_ui_state` | READ | yes | no |
| 155 | `form_context` | READ | yes | no |
| 156 | `network_context` | READ | yes | no |
| 157 | `console_context` | READ | yes | no |
| 158 | `browser_debug_context` | READ | yes | no |
| 159 | `service_context` | READ | yes | no |
| 160 | `process_context` | READ | yes | no |
| 161 | `port_context` | READ | yes | no |
| 162 | `installed_runtime_context` | READ | yes | no |
| 163 | `path_context` | READ | yes | no |
| 164 | `startup_context` | READ | yes | no |
| 165 | `mcp_discover` | READ | yes | no |
| 166 | `mcp_health` | READ | yes | no |
| 167 | `mcp_resources` | READ | yes | no |
| 168 | `task_create` | EXECUTE | no | no |
| 169 | `task_status` | READ | yes | no |
| 170 | `task_cancel` | EXECUTE | no | no |
| 171 | `task_result` | READ | yes | no |
| 172 | `task_list` | READ | yes | no |
| 173 | `delegate` | EXECUTE | no | no |
| 174 | `delegate_status` | READ | yes | no |
| 175 | `delegate_cancel` | EXECUTE | no | no |
| 176 | `delegate_result` | READ | yes | no |
| 177 | `parallel_delegate` | EXECUTE | no | no |
| 178 | `permission_check` | READ | yes | no |
| 179 | `permission_profile` | READ | yes | no |
| 180 | `live_logs_query` | READ | yes | no |
| 181 | `live_logs_status` | READ | yes | no |
| 182 | `telemetry_dashboard` | READ | yes | no |
| 183 | `context_economy_stats` | READ | yes | no |
| 184 | `execution_plan` | READ | yes | no |
| 185 | `repo_map` | READ | yes | no |
| 186 | `context_expand` | READ | yes | no |
| 187 | `recovery_status` | READ | yes | no |
| 188 | `tool_schema_list` | READ | yes | no |
| 189 | `tool_schema_register` | WRITE | no | no |
| 190 | `capabilities` | READ | yes | no |
| 191 | `tool_search` | READ | yes | no |
| 192 | `tool_dynamic_filter` | READ | yes | no |
| 193 | `tool_describe` | READ | yes | no |
| 194 | `tool_categories` | READ | yes | no |
| 195 | `tool_function_find` | READ | yes | no |
| 196 | `tool_aliases` | READ | yes | no |
| 197 | `mcp_hub` | READ | yes | no |
| 198 | `dev_context` | READ | yes | no |
| 199 | `recipe_catalog` | READ | yes | no |
| 200 | `capture_screenshot` | READ | yes | no |
| 201 | `compare_screenshot` | READ | yes | no |
| 202 | `dom_snapshot` | READ | yes | no |
| 203 | `layout_metadata` | READ | yes | no |
| 204 | `visual_context` | READ | yes | no |
...
| 216 | `git_worktree_spawn` | DANGEROUS | no | yes |
| 217 | `git_worktree_remove` | DANGEROUS | no | yes |
| 218 | `pdf_extract_tables` | READ | yes | no |
...
| 223 | `policy_explain` | READ | yes | no |
| 224 | `tool_batch` | DANGEROUS | no | yes |

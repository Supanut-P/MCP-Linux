...
| 68 | `support_bundle` | DANGEROUS | no | yes |
| 69 | `skills_list` | DANGEROUS | no | yes |
| 70 | `skills_read` | DANGEROUS | no | yes |
| 71 | `mcp_list` | DANGEROUS | no | yes |
| 72 | `mcp_describe` | DANGEROUS | no | yes |
| 73 | `mcp_call` | DANGEROUS | no | yes |
| 74 | `workspace_context` | READ | yes | no |
| 75 | `workspace_context_continue` | READ | yes | no |
| 76 | `workspace_full_scan` | READ | yes | no |
| 77 | `workspace_full_scan_continue` | READ | yes | no |
| 78 | `workspace_snapshot` | READ | yes | no |
| 79 | `search_all` | READ | yes | no |
| 80 | `read_many_files` | READ | yes | no |
| 81 | `read_file_page` | READ | yes | no |
| 82 | `read_file_page_continue` | READ | yes | no |
| 83 | `workspace_index` | READ | yes | no |
| 84 | `workspace_index_status` | READ | yes | no |
| 85 | `workspace_index_watch` | READ | yes | no |
| 86 | `workspace_index_stop` | READ | yes | no |
| 87 | `session_handoff` | READ | yes | no |
| 88 | `verify_incremental` | EXECUTE | no | no |
...
| 132 | `plugin_install` | DANGEROUS | no | yes |
...
| 149 | `service_context` | READ | yes | no |
| 150 | `process_context` | READ | yes | no |
| 151 | `port_context` | READ | yes | no |
| 152 | `installed_runtime_context` | READ | yes | no |
| 153 | `path_context` | READ | yes | no |
| 154 | `startup_context` | READ | yes | no |
| 155 | `mcp_discover` | READ | yes | no |
| 156 | `mcp_health` | READ | yes | no |
| 157 | `mcp_resources` | READ | yes | no |
| 158 | `task_create` | EXECUTE | no | no |
| 159 | `task_status` | READ | yes | no |
| 160 | `task_cancel` | EXECUTE | no | no |
| 161 | `task_result` | READ | yes | no |
| 162 | `task_list` | READ | yes | no |
| 163 | `delegate` | EXECUTE | no | no |
| 164 | `delegate_status` | READ | yes | no |
| 165 | `delegate_cancel` | EXECUTE | no | no |
| 166 | `delegate_result` | READ | yes | no |
| 167 | `parallel_delegate` | EXECUTE | no | no |
| 168 | `permission_check` | READ | yes | no |
| 169 | `permission_profile` | READ | yes | no |
| 170 | `live_logs_query` | READ | yes | no |
| 171 | `live_logs_status` | READ | yes | no |
| 172 | `telemetry_dashboard` | READ | yes | no |
| 173 | `context_economy_stats` | READ | yes | no |
| 174 | `execution_plan` | READ | yes | no |
| 175 | `repo_map` | READ | yes | no |
| 176 | `context_expand` | READ | yes | no |
| 177 | `recovery_status` | READ | yes | no |
| 178 | `tool_schema_list` | READ | yes | no |
| 179 | `tool_schema_register` | WRITE | no | no |
| 180 | `capabilities` | READ | yes | no |
| 181 | `tool_search` | READ | yes | no |
| 182 | `tool_dynamic_filter` | READ | yes | no |
| 183 | `tool_describe` | READ | yes | no |
| 184 | `tool_categories` | READ | yes | no |
| 185 | `tool_function_find` | READ | yes | no |
| 186 | `tool_aliases` | READ | yes | no |
| 187 | `mcp_hub` | READ | yes | no |
| 188 | `dev_context` | READ | yes | no |
| 189 | `recipe_catalog` | READ | yes | no |
| 190 | `capture_screenshot` | READ | yes | no |
| 191 | `compare_screenshot` | READ | yes | no |
| 192 | `dom_snapshot` | READ | yes | no |
| 193 | `layout_metadata` | READ | yes | no |
| 194 | `visual_context` | READ | yes | no |
| 195 | `inspect_pdf` | READ | yes | no |
| 196 | `compare_pdf_pages` | READ | yes | no |
| 197 | `project_profile_get` | READ | yes | no |
| 198 | `project_profile_set` | WRITE | no | no |
| 199 | `handoff_context` | READ | yes | no |
| 200 | `benchmark_run` | EXECUTE | no | no |
| 201 | `regression_report` | READ | yes | no |

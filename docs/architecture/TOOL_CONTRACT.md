...
This block is generated from the built `ToolRegistry`. Current count: **221 tools**.
...
| 71 | `db_inspect` | READ | yes | no |
| 72 | `db_query` | READ | yes | no |
| 73 | `target_catalog` | READ | yes | no |
| 74 | `support_bundle` | DANGEROUS | no | yes |
| 75 | `skills_list` | DANGEROUS | no | yes |
| 76 | `skills_read` | DANGEROUS | no | yes |
| 77 | `mcp_list` | DANGEROUS | no | yes |
| 78 | `mcp_describe` | DANGEROUS | no | yes |
| 79 | `mcp_call` | DANGEROUS | no | yes |
| 80 | `workspace_context` | READ | yes | no |
| 81 | `workspace_context_continue` | READ | yes | no |
| 82 | `workspace_full_scan` | READ | yes | no |
| 83 | `workspace_full_scan_continue` | READ | yes | no |
| 84 | `workspace_snapshot` | READ | yes | no |
| 85 | `search_all` | READ | yes | no |
| 86 | `read_many_files` | READ | yes | no |
| 87 | `read_file_page` | READ | yes | no |
| 88 | `read_file_page_continue` | READ | yes | no |
| 89 | `workspace_index` | READ | yes | no |
| 90 | `workspace_index_status` | READ | yes | no |
| 91 | `workspace_index_watch` | READ | yes | no |
| 92 | `workspace_index_stop` | READ | yes | no |
| 93 | `workspace_changes` | READ | yes | no |
| 94 | `session_handoff` | READ | yes | no |
| 95 | `verify_incremental` | EXECUTE | no | no |
| 96 | `symbol_search` | READ | yes | no |
| 97 | `find_definition` | READ | yes | no |
| 98 | `find_references` | READ | yes | no |
| 99 | `find_implementations` | READ | yes | no |
| 100 | `call_hierarchy` | READ | yes | no |
| 101 | `import_graph` | READ | yes | no |
| 102 | `dependency_graph` | READ | yes | no |
| 103 | `module_graph` | READ | yes | no |
| 104 | `type_search` | READ | yes | no |
| 105 | `trace_symbol` | READ | yes | no |
| 106 | `context_ranking` | READ | yes | no |
| 107 | `debug_context` | READ | yes | no |
| 108 | `review_context` | READ | yes | no |
| 109 | `change_context` | READ | yes | no |
| 110 | `symbol_context` | READ | yes | no |
| 111 | `test_context` | READ | yes | no |
| 112 | `dependency_context` | READ | yes | no |
| 113 | `git_context` | READ | yes | no |
| 114 | `frontend_context` | READ | yes | no |
| 115 | `backend_context` | READ | yes | no |
| 116 | `route_intent` | READ | yes | no |
| 117 | `recipe_list` | READ | yes | no |
| 118 | `recipe_describe` | READ | yes | no |
| 119 | `recipe_run` | EXECUTE | no | no |
| 120 | `dry_run` | READ | yes | no |
| 121 | `review_changes` | READ | yes | no |
| 122 | `changed_symbols` | READ | yes | no |
| 123 | `affected_modules` | READ | yes | no |
| 124 | `git_history_context` | READ | yes | no |
| 125 | `git_blame_context` | READ | yes | no |
| 126 | `discover_tests` | READ | yes | no |
| 127 | `run_affected_tests` | EXECUTE | no | no |
| 128 | `test_failures` | READ | yes | no |
| 129 | `coverage_context` | READ | yes | no |
| 130 | `test_history` | READ | yes | no |
| 131 | `cache_stats` | READ | yes | no |
| 132 | `cache_clear` | WRITE | no | no |
| 133 | `cache_invalidate` | WRITE | no | no |
| 134 | `hook_list` | READ | yes | no |
| 135 | `hook_register` | WRITE | no | no |
| 136 | `hook_remove` | WRITE | no | no |
| 137 | `skill_match` | READ | yes | no |
| 138 | `skill_load` | READ | yes | no |
| 139 | `plugin_install` | DANGEROUS | no | yes |
| 140 | `plugin_list` | READ | yes | no |
| 141 | `plugin_enable` | WRITE | no | no |
| 142 | `plugin_disable` | WRITE | no | no |
| 143 | `plugin_remove` | DANGEROUS | no | yes |
| 144 | `session_context` | READ | yes | no |
| 145 | `session_checkpoint` | WRITE | no | no |
| 146 | `session_resume` | READ | yes | no |
| 147 | `session_history` | READ | yes | no |
| 148 | `response_mode` | READ | yes | no |
| 149 | `inspect_web_app` | READ | yes | no |
| 150 | `debug_ui` | READ | yes | no |
| 151 | `capture_ui_state` | READ | yes | no |
| 152 | `form_context` | READ | yes | no |
| 153 | `network_context` | READ | yes | no |
| 154 | `console_context` | READ | yes | no |
| 155 | `browser_debug_context` | READ | yes | no |
| 156 | `service_context` | READ | yes | no |
| 157 | `process_context` | READ | yes | no |
| 158 | `port_context` | READ | yes | no |
...
| 209 | `lsp_diagnostics` | READ | yes | no |
| 210 | `lsp_rename` | WRITE | no | no |
| 211 | `debug_attach` | EXECUTE | no | no |
| 212 | `debug_step` | EXECUTE | no | no |
| 213 | `git_worktree_spawn` | DANGEROUS | no | yes |
| 214 | `git_worktree_remove` | DANGEROUS | no | yes |
| 215 | `pdf_extract_tables` | READ | yes | no |
| 216 | `self_heal_plan` | READ | yes | no |
| 217 | `self_heal_apply` | DANGEROUS | no | yes |
| 218 | `skills_import` | WRITE | no | no |
| 219 | `agent_swarm_run` | EXECUTE | no | no |
| 220 | `policy_explain` | READ | yes | no |
| 221 | `tool_batch` | DANGEROUS | no | yes |

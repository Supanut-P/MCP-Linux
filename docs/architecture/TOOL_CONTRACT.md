...
This block is generated from the built `ToolRegistry`. Current count: **216 tools**.
...
| 68 | `db_inspect` | READ | yes | no |
| 69 | `db_query` | READ | yes | no |
| 70 | `target_catalog` | READ | yes | no |
| 71 | `support_bundle` | DANGEROUS | no | yes |
| 72 | `skills_list` | DANGEROUS | no | yes |
| 73 | `skills_read` | DANGEROUS | no | yes |
| 74 | `mcp_list` | DANGEROUS | no | yes |
| 75 | `mcp_describe` | DANGEROUS | no | yes |
| 76 | `mcp_call` | DANGEROUS | no | yes |
| 77 | `workspace_context` | READ | yes | no |
| 78 | `workspace_context_continue` | READ | yes | no |
| 79 | `workspace_full_scan` | READ | yes | no |
| 80 | `workspace_full_scan_continue` | READ | yes | no |
| 81 | `workspace_snapshot` | READ | yes | no |
| 82 | `search_all` | READ | yes | no |
| 83 | `read_many_files` | READ | yes | no |
| 84 | `read_file_page` | READ | yes | no |
| 85 | `read_file_page_continue` | READ | yes | no |
| 86 | `workspace_index` | READ | yes | no |
| 87 | `workspace_index_status` | READ | yes | no |
| 88 | `workspace_index_watch` | READ | yes | no |
| 89 | `workspace_index_stop` | READ | yes | no |
| 90 | `session_handoff` | READ | yes | no |
| 91 | `verify_incremental` | EXECUTE | no | no |
| 92 | `symbol_search` | READ | yes | no |
| 93 | `find_definition` | READ | yes | no |
| 94 | `find_references` | READ | yes | no |
| 95 | `find_implementations` | READ | yes | no |
| 96 | `call_hierarchy` | READ | yes | no |
| 97 | `import_graph` | READ | yes | no |
| 98 | `dependency_graph` | READ | yes | no |
| 99 | `module_graph` | READ | yes | no |
| 100 | `type_search` | READ | yes | no |
| 101 | `trace_symbol` | READ | yes | no |
| 102 | `context_ranking` | READ | yes | no |
| 103 | `debug_context` | READ | yes | no |
| 104 | `review_context` | READ | yes | no |
| 105 | `change_context` | READ | yes | no |
| 106 | `symbol_context` | READ | yes | no |
| 107 | `test_context` | READ | yes | no |
| 108 | `dependency_context` | READ | yes | no |
| 109 | `git_context` | READ | yes | no |
| 110 | `frontend_context` | READ | yes | no |
| 111 | `backend_context` | READ | yes | no |
| 112 | `route_intent` | READ | yes | no |
| 113 | `recipe_list` | READ | yes | no |
| 114 | `recipe_describe` | READ | yes | no |
| 115 | `recipe_run` | EXECUTE | no | no |
| 116 | `dry_run` | READ | yes | no |
| 117 | `review_changes` | READ | yes | no |
| 118 | `changed_symbols` | READ | yes | no |
| 119 | `affected_modules` | READ | yes | no |
| 120 | `git_history_context` | READ | yes | no |
| 121 | `git_blame_context` | READ | yes | no |
| 122 | `discover_tests` | READ | yes | no |
| 123 | `run_affected_tests` | EXECUTE | no | no |
| 124 | `test_failures` | READ | yes | no |
| 125 | `coverage_context` | READ | yes | no |
| 126 | `test_history` | READ | yes | no |
| 127 | `cache_stats` | READ | yes | no |
| 128 | `cache_clear` | WRITE | no | no |
| 129 | `cache_invalidate` | WRITE | no | no |
| 130 | `hook_list` | READ | yes | no |
| 131 | `hook_register` | WRITE | no | no |
| 132 | `hook_remove` | WRITE | no | no |
| 133 | `skill_match` | READ | yes | no |
| 134 | `skill_load` | READ | yes | no |
| 135 | `plugin_install` | DANGEROUS | no | yes |
| 136 | `plugin_list` | READ | yes | no |
| 137 | `plugin_enable` | WRITE | no | no |
| 138 | `plugin_disable` | WRITE | no | no |
| 139 | `plugin_remove` | DANGEROUS | no | yes |
| 140 | `session_context` | READ | yes | no |
| 141 | `session_checkpoint` | WRITE | no | no |
| 142 | `session_resume` | READ | yes | no |
| 143 | `session_history` | READ | yes | no |
| 144 | `response_mode` | READ | yes | no |
| 145 | `inspect_web_app` | READ | yes | no |
| 146 | `debug_ui` | READ | yes | no |
| 147 | `capture_ui_state` | READ | yes | no |
| 148 | `form_context` | READ | yes | no |
| 149 | `network_context` | READ | yes | no |
| 150 | `console_context` | READ | yes | no |
| 151 | `browser_debug_context` | READ | yes | no |
...
| 198 | `inspect_pdf` | READ | yes | no |
| 199 | `compare_pdf_pages` | READ | yes | no |
| 200 | `project_profile_get` | READ | yes | no |
| 201 | `project_profile_set` | WRITE | no | no |
| 202 | `handoff_context` | READ | yes | no |
| 203 | `benchmark_run` | EXECUTE | no | no |
| 204 | `regression_report` | READ | yes | no |
| 205 | `lsp_diagnostics` | READ | yes | no |
| 206 | `lsp_rename` | WRITE | no | no |
| 207 | `debug_attach` | EXECUTE | no | no |
| 208 | `debug_step` | EXECUTE | no | no |
| 209 | `git_worktree_spawn` | DANGEROUS | no | yes |
| 210 | `git_worktree_remove` | DANGEROUS | no | yes |
| 211 | `pdf_extract_tables` | READ | yes | no |
| 212 | `self_heal_plan` | READ | yes | no |
| 213 | `self_heal_apply` | DANGEROUS | no | yes |
| 214 | `skills_import` | WRITE | no | no |
| 215 | `agent_swarm_run` | EXECUTE | no | no |
| 216 | `tool_batch` | DANGEROUS | no | yes |

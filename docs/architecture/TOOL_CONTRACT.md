...
| 60 | `remote_rollout` | DANGEROUS | no | yes |
| 61 | `artifact_verify` | READ | yes | no |
| 62 | `http_probe` | READ | yes | no |
| 63 | `storage_usage` | READ | yes | no |
| 64 | `backup` | DANGEROUS | no | yes |
| 65 | `db_inspect` | READ | yes | no |
| 66 | `db_query` | READ | yes | no |
| 67 | `target_catalog` | READ | yes | no |
| 68 | `skills_list` | DANGEROUS | no | yes |
| 69 | `skills_read` | DANGEROUS | no | yes |
| 70 | `mcp_list` | DANGEROUS | no | yes |
| 71 | `mcp_describe` | DANGEROUS | no | yes |
| 72 | `mcp_call` | DANGEROUS | no | yes |
| 73 | `workspace_context` | READ | yes | no |
| 74 | `workspace_context_continue` | READ | yes | no |
| 75 | `workspace_full_scan` | READ | yes | no |
| 76 | `workspace_full_scan_continue` | READ | yes | no |
| 77 | `workspace_snapshot` | READ | yes | no |
| 78 | `search_all` | READ | yes | no |
| 79 | `read_many_files` | READ | yes | no |
| 80 | `read_file_page` | READ | yes | no |
| 81 | `read_file_page_continue` | READ | yes | no |
| 82 | `workspace_index` | READ | yes | no |
| 83 | `workspace_index_status` | READ | yes | no |
| 84 | `workspace_index_watch` | READ | yes | no |
| 85 | `workspace_index_stop` | READ | yes | no |
| 86 | `session_handoff` | READ | yes | no |
| 87 | `verify_incremental` | EXECUTE | no | no |
| 88 | `symbol_search` | READ | yes | no |
| 89 | `find_definition` | READ | yes | no |
| 90 | `find_references` | READ | yes | no |
| 91 | `find_implementations` | READ | yes | no |
| 92 | `call_hierarchy` | READ | yes | no |
| 93 | `import_graph` | READ | yes | no |
| 94 | `dependency_graph` | READ | yes | no |
| 95 | `module_graph` | READ | yes | no |
| 96 | `type_search` | READ | yes | no |
| 97 | `trace_symbol` | READ | yes | no |
| 98 | `context_ranking` | READ | yes | no |
| 99 | `debug_context` | READ | yes | no |
| 100 | `review_context` | READ | yes | no |
| 101 | `change_context` | READ | yes | no |
| 102 | `symbol_context` | READ | yes | no |
| 103 | `test_context` | READ | yes | no |
| 104 | `dependency_context` | READ | yes | no |
| 105 | `git_context` | READ | yes | no |
| 106 | `frontend_context` | READ | yes | no |
| 107 | `backend_context` | READ | yes | no |
| 108 | `route_intent` | READ | yes | no |
| 109 | `recipe_list` | READ | yes | no |
| 110 | `recipe_describe` | READ | yes | no |
| 111 | `recipe_run` | EXECUTE | no | no |
| 112 | `dry_run` | READ | yes | no |
| 113 | `review_changes` | READ | yes | no |
| 114 | `changed_symbols` | READ | yes | no |
| 115 | `affected_modules` | READ | yes | no |
| 116 | `git_history_context` | READ | yes | no |
| 117 | `git_blame_context` | READ | yes | no |
| 118 | `discover_tests` | READ | yes | no |
| 119 | `run_affected_tests` | EXECUTE | no | no |
| 120 | `test_failures` | READ | yes | no |
| 121 | `coverage_context` | READ | yes | no |
| 122 | `test_history` | READ | yes | no |
| 123 | `cache_stats` | READ | yes | no |
| 124 | `cache_clear` | WRITE | no | no |
| 125 | `cache_invalidate` | WRITE | no | no |
| 126 | `hook_list` | READ | yes | no |
| 127 | `hook_register` | WRITE | no | no |
| 128 | `hook_remove` | WRITE | no | no |
| 129 | `skill_match` | READ | yes | no |
| 130 | `skill_load` | READ | yes | no |
| 131 | `plugin_install` | DANGEROUS | no | yes |
| 132 | `plugin_list` | READ | yes | no |
| 133 | `plugin_enable` | WRITE | no | no |
| 134 | `plugin_disable` | WRITE | no | no |
| 135 | `plugin_remove` | DANGEROUS | no | yes |
| 136 | `session_context` | READ | yes | no |
| 137 | `session_checkpoint` | WRITE | no | no |
| 138 | `session_resume` | READ | yes | no |
| 139 | `session_history` | READ | yes | no |
| 140 | `response_mode` | READ | yes | no |
| 141 | `inspect_web_app` | READ | yes | no |
| 142 | `debug_ui` | READ | yes | no |
| 143 | `capture_ui_state` | READ | yes | no |
| 144 | `form_context` | READ | yes | no |
| 145 | `network_context` | READ | yes | no |
| 146 | `console_context` | READ | yes | no |
| 147 | `browser_debug_context` | READ | yes | no |
| 148 | `service_context` | READ | yes | no |
| 149 | `process_context` | READ | yes | no |
| 150 | `port_context` | READ | yes | no |
...
| 194 | `inspect_pdf` | READ | yes | no |
| 195 | `compare_pdf_pages` | READ | yes | no |
| 196 | `project_profile_get` | READ | yes | no |
| 197 | `project_profile_set` | WRITE | no | no |
| 198 | `handoff_context` | READ | yes | no |
| 199 | `benchmark_run` | EXECUTE | no | no |
| 200 | `regression_report` | READ | yes | no |
| 201 | `lsp_diagnostics` | READ | yes | no |
| 202 | `lsp_rename` | WRITE | no | no |
| 203 | `debug_attach` | EXECUTE | no | no |
| 204 | `debug_step` | EXECUTE | no | no |
| 205 | `git_worktree_spawn` | DANGEROUS | no | yes |
| 206 | `git_worktree_remove` | DANGEROUS | no | yes |

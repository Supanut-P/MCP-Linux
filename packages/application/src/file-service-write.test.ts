...
import type { Checkpoint, Workspace, WorkspaceRepository } from '@baitonghub-linux-mcp/workspace';
...
async function createWorkspace(): Promise<Workspace> {
  const rawRoot = await mkdtemp(path.join(os.tmpdir(), 'baitonghub-linux-mcp-file-write-'));
...
describe('FileService writes', () => {
  it('does not write after cancellation wins during checkpoint creation', async () => {
    ...
    const writing = service.writeFile(actor, workspace.id, { path: path.join('src', 'file.txt'), content: 'after' }, controller.signal);
  ...
  it('returns PERMISSION_REQUIRED under Safe and leaves the file unchanged', async () => {
    ...
    }).writeFile(actor, workspace.id, { path: path.join('src', 'file.txt'), content: 'after' });
  ...
  it('creates a checkpoint and moves delete_file targets into recovery trash', async () => {
    const workspace = await createWorkspace();
    const recoveryRoot = await mkdtemp(path.join(os.tmpdir(), 'baitonghub-linux-mcp-recovery-'));
  ...
  it('moves a file into a nested destination that does not exist yet', async () => {
    ...
    const result = await new FileService(repository(workspace), undefined, undefined, { checkpointService: checkpointService() })
      .moveFile(actor, workspace.id, { sourcePath: path.join('src', 'file.txt'), destinationPath: path.join('docs', 'moved', 'file.txt') });
  ...
  it('copies files and directories into missing destination parents', async () => {
    ...
    await expect(service.moveFile(actor, workspace.id, { sourcePath: path.join('src', 'pkg'), destinationPath: path.join('relocated', 'pkg') }))

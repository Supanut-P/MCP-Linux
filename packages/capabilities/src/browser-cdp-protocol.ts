...
import { PathExecutableResolver, createProcessTreeTerminator, type ProcessTreeTerminator } from '@baitonghub-linux-mcp/process';
...
export class NodeBrowserCdpProtocol implements BrowserCdpProtocol {
  ...
  public constructor(options: BrowserCdpProtocolOptions = {}) {
    this.port = options.port ?? readPort(process.env.BAITONGHUB_LINUX_MCP_BROWSER_CDP_PORT);
    this.profileDir = options.profileDir ?? process.env.BAITONGHUB_LINUX_MCP_BROWSER_PROFILE ?? path.join(os.tmpdir(), 'baitonghub-linux-mcp-browser-profile');
    this.chromeExecutable = options.chromeExecutable ?? process.env.BAITONGHUB_LINUX_MCP_BROWSER_EXECUTABLE;
  ...
  private async findChromeExecutable(): Promise<string | undefined> {
    ...
    const resolver = new PathExecutableResolver();

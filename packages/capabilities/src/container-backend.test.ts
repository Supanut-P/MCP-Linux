...
describe('ContainerBackend', () => {
  it('prefers Docker and uses fixed argv for list/logs', async () => {
    ...
    const backend = new ContainerBackend({ platform: 'linux', allowedRootsProvider: async (): Promise<readonly string[]> => ['/tmp'], resolveExecutable: async (name: string): Promise<string> => `/usr/bin/${name}`, runner });
    await expect(backend.execute({ operation: 'list', all: true, workspaceId: 'workspace-1', project_root: '/tmp' })).resolves.toMatchObject({ ok: true, value: { provider: 'docker' } });
    await expect(backend.execute({ operation: 'logs', container: 'api', tail: 200, workspaceId: 'workspace-1', project_root: '/tmp' })).resolves.toMatchObject({ ok: true });
    expect(calls).toEqual([
      expect.objectContaining({ executable: '/usr/bin/docker', args: ['ps', '--all'], options: expect.objectContaining({ shell: false }) }),
      expect.objectContaining({ executable: '/usr/bin/docker', args: ['logs', '--tail', '200', 'api'], options: expect.objectContaining({ shell: false }) }),
    ]);
  });

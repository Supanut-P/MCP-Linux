...
describe('LocalCapabilityService', () => {
  it('dispatches each Linux capability to its local backend', async () => {
    const calls: string[] = [];
  ...
  it('exposes the platform-filtered list used by MCP tools/list', () => {
    const backend = { execute: async (): Promise<ReturnType<typeof ok>> => ok({}) };
    const service = new LocalCapabilityService({
      shell: backend,
      domCdp: backend,
      accessibility: backend,
    ...
    expect(service.listTools()).toEqual(['shell', 'health']);
  });
});

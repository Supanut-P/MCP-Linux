...
describe.runIf(process.platform === 'linux')('ShellCapabilityBackend', () => {
  it('runs an executable with separate arguments and returns bounded output', async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), 'baitonghub-linux-mcp-shell-'));
  ...
  it('rejects a working directory outside configured local roots', async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), 'baitonghub-linux-mcp-shell-'));
    const outside = await mkdtemp(path.join(os.tmpdir(), 'baitonghub-linux-mcp-shell-outside-'));
...
describe.runIf(process.platform === 'linux')('ShellCapabilityBackend unrestricted', () => {
  ...
  it('passes the full environment through in unrestricted mode', async () => {
    ...
    const result = await backend.execute({
      operation: 'run',
      executable: process.execPath,
      arguments: ['-e', "process.stdout.write(process.env.BAITONGHUB_LINUX_MCP_ENV_PROBE ?? 'missing')"],
      cwd: root,
      execution: 'foreground',
    });
  ...
  it('persists durable task ownership and rejects another session in the same workspace', async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), 'baitonghub-linux-mcp-shell-owner-'));
    temporaryRoots.push(root);

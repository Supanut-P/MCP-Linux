...
export class ShellCapabilityBackend implements CapabilityBackend {
  ...
  private async run(request: ShellRequest, signal?: AbortSignal): Promise<Result<unknown>> {
    ...
    if (request.dryRun) {
      return ok({ dry_run: true, executable: executable.value, arguments: [...request.arguments], cwd: cwd.value });
    }

    if (this.durableStore !== undefined && request.execution !== 'foreground') {
      return this.runDurable(request, cwd.value, { executable: executable.value, args: request.arguments });
    }

    let child: ChildProcess;
    try {
      child = spawn(executable.value, [...request.arguments], {
        cwd: cwd.value,
...
function createSafeEnvironment(source: NodeJS.ProcessEnv, unrestricted: boolean): NodeJS.ProcessEnv {
  ...
  const allowed = new Set(['PATH', 'HOME', 'LANG', 'LC_ALL', 'LC_CTYPE', 'TMPDIR', 'XDG_DATA_HOME', 'XDG_CONFIG_HOME', 'XDG_STATE_HOME', 'XDG_RUNTIME_DIR', 'DBUS_SESSION_BUS_ADDRESS', 'DISPLAY', 'WAYLAND_DISPLAY']);
  return Object.fromEntries(Object.entries(source).filter(([key, entry]) => {
    return entry !== undefined && allowed.has(key);
  }));
}
...
function isDeleteLikeShellCommand(executable: string, args: readonly string[]): boolean {
  ...
  const deleteNames = new Set(['rm', 'rmdir', 'unlink']);
  if (deleteNames.has(basename)) return true;
  const joined = args.map((entry) => entry.toLowerCase()).join(' ');
  if (['bash', 'sh', 'dash', 'zsh'].includes(basename)) return /(^|[;&|\s])(rm|unlink|rmdir|truncate)\b/.test(joined);
  return false;
}

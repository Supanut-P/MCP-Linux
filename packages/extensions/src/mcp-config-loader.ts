...
export class McpConfigLoader {
  ...
  public async discover(): Promise<readonly DiscoveredMcpServer[]> {
    ...
    for (const [name, config] of Object.entries(this.options.settings.extraMcpServers)) {
      discovered.push(this.toServer(name, 'baitonghub-linux-mcp-settings', config));
    }
...
export function exclusionReason(name: string, config: McpServerLaunchConfig): string | undefined {
  ...
  if (lowered === 'baitonghub-linux-mcp' || lowered.startsWith('baitonghub-linux-mcp-')) {
    return 'Refusing to aggregate baitonghub-linux-mcp itself';
  }
  const command = path.basename(config.command).toLowerCase();
  if (command === 'baitonghub-linux-mcp' || command.includes('baitonghub-linux-mcp')) {
    return 'Refusing to aggregate baitonghub-linux-mcp itself';
  }
  const args = (config.args ?? []).join(' ').toLowerCase();
  if (args.includes('--mcp-stdio') && command.includes('baitonghub-linux-mcp')) {
    return 'Refusing to aggregate baitonghub-linux-mcp itself';
  }
  return undefined;
}

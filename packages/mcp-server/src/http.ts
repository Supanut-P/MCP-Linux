...
export interface McpHttpServerOptions extends McpServerOptions {
  ...
  /** Bind address. Loopback is the safe default; non-loopback binds require auth in the CLI. */
  readonly host?: string;
  /** Host header values accepted by the MCP boundary. */
  readonly allowedHostnames?: readonly string[];
  /** Optional bearer token checked before an MCP request is dispatched. */
...
function writeDiagnostic(error: Error): void {
  process.stderr.write(`baitonghub-linux-mcp MCP HTTP error: ${error.message}\n`);
}
...
function toFetchRequest(request: IncomingMessage, body: Buffer, host: string): Request {
  ...
  const requestedPath = new URL(request.url ?? '/', `http://${host}`);
  const init: RequestInit = { method: request.method ?? 'GET', headers };
  if (init.method !== 'GET' && init.method !== 'HEAD' && body.length > 0) {
    init.body = new Uint8Array(body);
  }
  return new Request(`http://${host}${requestedPath.pathname}${requestedPath.search}`, init);
}
...
async function handleRequest(
  ...
  if (authToken !== undefined) {
    ...
    if (authorization !== `Bearer ${authToken}`) {
      sendStatus(response, 401, 'Unauthorized');
      return;
    }
  }
  ...
  const fetchRequest = toFetchRequest(request, read.body, host);
...
function listen(server: HttpServer, host: string, port: number): Promise<McpHttpServerAddress> {
...
export async function startMcpHttp(options: McpHttpServerOptions): Promise<McpHttpServerHandle> {
  ...
  const allowedHostnames = options.allowedHostnames === undefined || options.allowedHostnames.length === 0
    ? localhostAllowedHostnames()
    : [...new Set(options.allowedHostnames.map((value) => value.trim()).filter((value) => value.length > 0))];
  if (allowedHostnames.length === 0) throw new Error('MCP HTTP allowed hostnames must not be empty');
  ...
  const server = createServer((request, response) => {
    void handleRequest(request, response, handler, originPolicy, maxBodyBytes, host, allowedHostnames, options.authToken).catch((error: unknown) => {
  ...
  try {
    address = await listen(server, host, options.port);
  } catch (error: unknown) {

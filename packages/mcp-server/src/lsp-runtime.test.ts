...
describe('LspRuntimeService', () => {
  it('reports the missing configuration truthfully before spawning anything', async () => {
    let spawns = 0;
    const runtime = new LspRuntimeService(servicesWithRoot(path.resolve('fixture-ws')), actor, {
      environment: {},
    ...
    await expect(runtime.diagnostics({ workspaceId: 'ws-1', files: ['src/a.ts'] })).resolves.toMatchObject({
      ok: false, error: { code: 'PERMISSION_DENIED', message: expect.stringContaining('BAITONGHUB_LINUX_MCP_LSP_TYPESCRIPT_COMMAND') },
    });
  ...
  it('collects published diagnostics from a configured language server', async () => {
    ...
    try {
      const runtime = new LspRuntimeService(servicesWithRoot(root), actor, {
        environment: { BAITONGHUB_LINUX_MCP_LSP_TYPESCRIPT_COMMAND: JSON.stringify([process.execPath, fakeServer]) },
        timeoutMs: 10_000,
      });
...
async function createFakeServer(): Promise<string> {
  ...
  const directory = await mkdtemp(path.join(tmpdir(), 'baitonghub-linux-mcp-lsp-fake-'));

 ...
 * (BAITONGHUB_LINUX_MCP_LSP_<LANGUAGE>_COMMAND, JSON argv preferred). Workspace files are
...
export class LspRuntimeService {
  ...
  private async startSession(
    ...
    if (command === undefined) {
      return err(appError('PERMISSION_DENIED', `No language server configured for ${language}. Set BAITONGHUB_LINUX_MCP_LSP_${language.toUpperCase()}_COMMAND (JSON argv preferred)`));
    }
  ...
  private serverCommand(language: string): readonly string[] | undefined {
    ...
    const configured = readString(settingsCommand) ?? this.environment[`BAITONGHUB_LINUX_MCP_LSP_${language.toUpperCase()}_COMMAND`];
...
function defaultSpawner(command: readonly string[]): Result<ChildProcess> {
  try {
    return ok(spawn(command[0]!, [...command.slice(1)], { stdio: ['pipe', 'pipe', 'pipe'] }));
  } catch {

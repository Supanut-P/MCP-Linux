...
const LEGACY_SNAPSHOT_FILE = 'baitonghub-linux-mcp.mcp.activity.json';
const LEASE_DIRECTORY = 'baitonghub-linux-mcp.mcp.activity.v2';
...
export async function probeProcessStart(pid: number, options: ProcessProbeOptions = {}): Promise<ProcessProbeResult> {
  ...
  const runProbe = options.runProbe ?? runPlatformProcessProbe;
...
async function runPlatformProcessProbe(pid: number, timeoutMs: number): Promise<string> {
  if (process.platform !== 'linux') throw new Error('process_probe_platform_unsupported');
  try {
    ...
    const value = stdout.trim();
    if (value.length === 0) return 'GONE';
    const startedAt = new Date(value);
    if (Number.isNaN(startedAt.getTime())) throw new Error('process_probe_invalid_start');
    return `LIVE|${startedAt.toISOString()}`;
  } catch (error: unknown) {
    if (isRecord(error) && error.code === 1) return 'GONE';
    throw error;
  }
}

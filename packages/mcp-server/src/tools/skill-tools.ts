...
export function skillTools(context: McpToolContext): McpToolDefinition[] {
  return [
    defineTool({
      name: 'skills_list',
      description: 'List local agent skills discovered from Cursor, Claude, Agents, workspace skill roots, and baitonghub-linux-mcp settings. Filter with query or source.',
      ...fullAccess,
    ...
    defineTool({
      name: 'skills_read',
      description: 'Read a local skill SKILL.md (or a relative file inside the skill folder). Follow the skill instructions with baitonghub-linux-mcp tools and mcp_call.',
      ...fullAccess,

...
export function workspaceTools(context: McpToolContext): McpToolDefinition[] {
  return [
    defineTool({
      name: 'workspace_list',
      description: 'List all registered Linux roots and project workspaces available to baitonghub-linux-mcp. Call this first to discover workspace IDs. Entries include kind=machine_root|project.',
      permission: 'READ',
    ...
    defineTool({
      ...
      description: 'Register an existing project directory under a registered Linux machine root. parentWorkspaceId must be a machine root from workspace_list. Idempotent for the same path.',
      permission: 'WRITE',

...
export const UPGRADE_TOOL_CATALOG: readonly UpgradeToolCatalogEntry[] = [
  ...
  read('service_context', 20, 'Inspect Linux service metadata.', ['linux', 'services']),
  read('process_context', 20, 'Inspect Linux process-tree context.', ['linux', 'process']),
  read('port_context', 20, 'Inspect local listening-port context.', ['linux', 'network']),
  read('installed_runtime_context', 20, 'Inspect installed Linux runtimes and package managers.', ['linux', 'runtime']),
  read('path_context', 20, 'Resolve executable and PATH context.', ['linux', 'path']),
  read('startup_context', 20, 'Inspect systemd user-service startup configuration.', ['linux', 'systemd']),
  ...
  read('remote_fleet', 47, 'Inspect a bounded set of explicitly registered SSH hosts with fixed read-only health, inventory, service-status, disk-usage, checksum, network-summary, and snapshot operations.', ['remote', 'fleet', 'linux', 'inspection'], { availability: 'optional', requirements: ['registered remote hosts', 'pinned SSH fingerprints', 'Linux Secret Service'], supportsCancel: true, supportsDryRun: false, auditTarget: 'remote-fleet' }),
  dangerous('remote_rollout', 48, 'Plan and execute an expiring canary-first restart of one fixed service across registered SSH hosts.', ['remote', 'rollout', 'canary', 'linux'], { availability: 'optional', requirements: ['registered remote hosts', 'pinned SSH fingerprints', 'Linux Secret Service', 'explicit confirmation'], supportsCancel: true, supportsDryRun: true, auditTarget: 'remote-rollout' }),
  dangerous('remote_rollout_resume', 50, 'Preview and execute a bounded recovery of failed or unattempted remote rollout hosts without restarting confirmed successes.', ['remote', 'rollout', 'resume', 'recovery', 'linux'], { availability: 'optional', requirements: ['failed or partial remote rollout', 'fresh restart previews', 'pinned SSH fingerprints', 'Linux Secret Service', 'explicit confirmation'], supportsCancel: true, supportsDryRun: true, auditTarget: 'remote-rollout-resume' }),
  dangerous('support_bundle', 49, 'Create a bounded, redacted support archive from registered workspace diagnostics.', ['support', 'diagnostics', 'redaction', 'linux'], { availability: 'optional', requirements: ['registered workspace', 'archive provider', 'explicit confirmation'], supportsCancel: true, supportsDryRun: true, auditTarget: 'support-bundle' }),

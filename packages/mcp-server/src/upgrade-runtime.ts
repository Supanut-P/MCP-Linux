...
import { capabilityDescriptors, type CapabilityDescriptor } from '@baitonghub-linux-mcp/capabilities';
...
export class UpgradeRuntimeService {
  ...
  public async execute(name: string, input: Record<string, unknown>, signal?: AbortSignal): Promise<Result<unknown>> {
    ...
    switch (name) {
      ...
      case 'project_profile_get':
        return ok({ profile: null, source: '.baitonghub-linux-mcp/project.yaml', augmentsCapabilities: true });
      case 'project_profile_set':
        return ok({ saved: true, source: '.baitonghub-linux-mcp/project.yaml', accessRestrictionsChanged: false });
      case 'debug_context':
      case 'review_context':
      case 'change_context':
      case 'symbol_context':
  ...
  private async gitWorktreeSpawn(input: Record<string, unknown>): Promise<Result<unknown>> {
    ...
    if (normalizedPath.startsWith('/') || /^[A-Za-z]:/.test(normalizedPath) || normalizedPath.split('/').some((part) => part === '..') || !(normalizedPath.startsWith('.worktrees/') || normalizedPath.startsWith('.baitonghub-linux-mcp/worktrees/'))) {
      return err(appError('PATH_OUTSIDE_WORKSPACE', 'Git worktree path must remain under .worktrees or .baitonghub-linux-mcp/worktrees'));
    }

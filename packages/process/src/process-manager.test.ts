...
import { ProcessManager, type ExecutableResolver, type ProcessTreeTerminator } from './index.js';
...
describe('ProcessManager', () => {
  ...
  it.runIf(process.platform === 'linux')('times out a running child and stops only an owned process group', async () => {
    const manager = new ProcessManager();
    const started = await manager.start({
      executable: process.execPath,
  ...
  it('returns EXECUTABLE_NOT_FOUND without accepting an arbitrary shell command', async () => {
    const result = await new ProcessManager().start({
      executable: 'baitonghub-linux-mcp-executable-that-does-not-exist',
      args: ['&&', 'whoami'],
      cwd: process.cwd(),
    });
  ...
  it('does not spawn after cancellation wins during executable resolution', async () => {
    const root = await mkdtemp(path.join(process.cwd(), 'baitonghub-linux-mcp-cancelled-process-'));

...
export type DatabaseTargetRegistration = Omit<DatabaseTarget, 'createdAt'> & { readonly createdAt?: string };

export class SqliteDatabaseTargetRepository {
  public constructor(private readonly database: SqliteDatabase) {}

  public async list(): Promise<readonly DatabaseTarget[]> {
    return this.rows(this.database.connection.prepare('SELECT id, display_name, driver, host, port, database_name, username, secret_ref, created_at FROM database_targets ORDER BY created_at, id').all());
  }

  public async get(id: string): Promise<DatabaseTarget | null> {
    return this.toTarget(this.database.connection.prepare('SELECT id, display_name, driver, host, port, database_name, username, secret_ref, created_at FROM database_targets WHERE id = ?').get(id));
  }

  public async insert(target: DatabaseTargetRegistration): Promise<void> {
    validateDatabaseTarget(target);
    this.database.connection.prepare('INSERT INTO database_targets (id, display_name, driver, host, port, database_name, username, secret_ref, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(
      target.id, target.displayName, target.driver, target.host, target.port, target.databaseName, target.username, target.secretRef, target.createdAt ?? new Date().toISOString(),
    );
  }
  ...
  private toTarget(value: unknown): DatabaseTarget | null {
    if (!isRecord(value) || typeof value.id !== 'string' || typeof value.display_name !== 'string' || typeof value.driver !== 'string'
      ...
      || typeof value.database_name !== 'string' || typeof value.username !== 'string' || typeof value.secret_ref !== 'string' || typeof value.created_at !== 'string') return null;
    return { id: value.id, displayName: value.display_name, driver: value.driver, host: value.host, port: value.port, databaseName: value.database_name, username: value.username, secretRef: value.secret_ref, createdAt: value.created_at };
  }
}

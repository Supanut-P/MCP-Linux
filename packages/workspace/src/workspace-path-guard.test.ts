...
async function createWorkspace(): Promise<Workspace> {
  const rawRoot = await mkdtemp(path.join(os.tmpdir(), 'baitonghub-linux-mcp-workspace-'));
...
describe('WorkspacePathGuard', () => {
  ...
  it('rejects traversal and absolute paths outside the workspace', async () => {
    ...
    expectError(await guard.resolveForRead(workspace, path.join('..', '..', 'outside.txt')), 'PATH_OUTSIDE_WORKSPACE');
  ...
  it('allows secret files inside the registered workspace in unrestricted mode', async () => {
    const workspace = await createWorkspace();

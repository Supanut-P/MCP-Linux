...
describe('v1 stable MCP contract', () => {
  it('matches the reviewed canonical tools/list fixture', async () => {
    ...
    const registry = new ToolRegistry({
      ...
      environmentPreflight: { execute: async (): Promise<Result<unknown>> => ({ ok: true, value: { operation: 'environment_preflight', status: 'ready', platform: 'linux', displayServer: 'headless', runtime: { nodeVersion: 'v24.0.0', nodeMajor: 24 }, capabilities: { total: 0, available: 0, ready: 0, consentRequired: 0, notReady: [], missingDependencies: [] } } }) },
      workflowPreflight: { execute: async (): Promise<Result<unknown>> => ({ ok: true, value: { operation: 'workflow_preflight', status: 'ready', environment: { available: true, ready: true, status: 'ready', platform: 'linux', displayServer: 'headless', runtime: { nodeVersion: 'v24.0.0', nodeMajor: 24 }, capabilities: { total: 0, available: 0, ready: 0, consentRequired: 0, notReady: [], missingDependencies: [] } }, diagnostics: { available: true, ready: true, status: 'ready', health: { available: true, ready: true, unavailableCount: 0, consentRequiredCount: 0, missingDependencies: [] }, runtime: { available: true, ready: true }, audit: { available: true, ready: true, count: 0, truncated: false }, dependencies: { ready: true, missingDependencies: [] } } } }) },
      workspaceCheckpoint: { execute: async (): Promise<Result<unknown>> => ({ ok: true, value: { operation: 'list', checkpoints: [], count: 0, truncated: false } }) },

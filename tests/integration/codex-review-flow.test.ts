...
import { CodexAdapter, type CodexDiscoveryPort, type CodexInvocationBuilderPort } from '@baitonghub-linux-mcp/codex';
import { ok, type Result } from '@baitonghub-linux-mcp/domain';
import { ProcessManager } from '@baitonghub-linux-mcp/process';
import { ToolRegistry, type McpApplicationServices } from '@baitonghub-linux-mcp/mcp-server';
import { SqliteAuditRepository, SqliteDatabase, SqliteWorkspaceRepository } from '@baitonghub-linux-mcp/storage';
...
describe.runIf(process.platform === 'linux')('Codex review flow', () => {
  it('delegates to a fake Codex executable, reviews the diff, runs the project test, and stops an owned task', async () => {
    const fixtureRoot = await createFixture();
    const rawStateRoot = await mkdtemp(path.join(os.tmpdir(), 'baitonghub-linux-mcp-codex-flow-state-'));
...
async function createFixture(): Promise<string> {
  const rawRoot = await mkdtemp(path.join(os.tmpdir(), 'baitonghub-linux-mcp-codex-flow-fixture-'));
  temporaryRoots.push(rawRoot);
  const root = await realpath(rawRoot);
  await mkdir(path.join(root, 'src'));
  await writeFile(path.join(root, 'src', 'reviewed.ts'), 'export const reviewed = false;\n', 'utf8');
  await writeFile(path.join(root, 'package.json'), JSON.stringify({ name: 'baitonghub-linux-mcp-codex-fixture', scripts: { test: 'node -e "process.stdout.write(\'project-test-pass\\n\')"' } }), 'utf8');
  ...
  await execFileAsync('git', ['config', 'user.name', 'baitonghub-linux-mcp codex integration'], { cwd: root, windowsHide: true });
